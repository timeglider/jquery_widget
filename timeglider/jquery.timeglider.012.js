...
(function($, undefined){
  ...
  $.fn.timeglider = function(params) {
		...
		********************************/
		...
		TimegliderModel.prototype = {
			...
			loadTimelineData : function (src, type) {
					...
					$.getJSON(src, function(data){
						...
						for (var i=0; i<data.length;i++) {
							...
							var t = new TimegliderTimeline(data[i]); // the timeline		
			...
			* creates hashbase of events indexed by unit serial
			...
			TODO ==> re-chew function for renewing stuff like startSeconds, etc
			     ==> uh... Timeline really needs to be it's own class with methods...
			*/
			chewTimeline : function (tdata, init) {
					...
					if (tdata.events) {
						...
						for(ei=0; ei< l; ei++) {
							...
							if (ev.id == 1462 || ev.id== 1463){
							trace ("ev:" + ev.title + "=" + ev.basicWidth + "px");
							}
		...
		VIEW
		*****************************************/
		function TimelineView (_model, settings) {
		...
		TimelineView.prototype = {
			...
			registerTitles : function () {
				...
				---- target only spanning events, not all events...
				*/
			...
			*/
			castTicks: function () {
				...
				var nTicks = Math.ceil(this.dimensions.width / tickWidth) + 4,
				    leftright = 'l';
				// ALTERNATING L & R
			...
			*/
			appendTimelines : function (tick) {
					...
					var tl, ev, posx, expCol, ht, borg, stuff, impq,
						foSec = M._startSec, 
						spp = M.getZoomInfo().spp,
						zl = M.getZoomInfo().level;
