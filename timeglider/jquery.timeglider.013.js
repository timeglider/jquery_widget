...
;(function($){
  ...
  $.fn.timeglider = function(params) {
		...
		/* 
		TODO Use this to set settings defaults, too 
		...
		*/	
		...
		function validateSettings(stgs) {	
			var ret = true,
				settingsTypes = settingsMaster;
			...
			$.each(stgs, function(key, value) { 
				...
				if (settingsTypes[key]) {
					//trace ("key:" + key + ", type:" + settingsTypes[key].type);
					switch (settingsTypes[key].type) {
						case "string": 
							if (typeof value != "string") msg += (key + " needs to be a string." + lb);
							if (settingsTypes[key].possible) {
								if ($.inArray(value, settingsTypes[key].possible) == -1) {
									msg += (key + " must be: " + settingsTypes[key].possible.join(" or "));
								}
							}
						break;
						...
						case "number":
							...
							if (settingsTypes[key].min) {
								if (value < settingsTypes[key].min) {
									msg += (key + " must be greater than " + settingsTypes[key].min + lb);
								}
							}
							
							if (settingsTypes[key].max) {
								if (value > settingsTypes[key].max) {
									msg += (key + " must be less than " + settingsTypes[key].max + lb);
								}
							}
						break;
						
						case "date":
							// validate a date string using TG_Date...
						break;
						...
						case "url":
							// test for pattern for url....
						break;
						
						case "color":
							/// test for pattern for color, including "red", "orange", etc
						break;
						
						default: trace ("is there a default for validating settings?");
		...
		*/
		...
		this.setZoom = function (z) {
			n = parseInt(z, 10);
			if (n <=100 && n>0) { model.setZoomLevel(n); }
		};
		...
		*/
			...
			*/
			chewTimeline : function (tdata, init) {
					...
					var units = tgDate.units; 
						...
						// TODO: VALIDATE COLOR, centralize default color(settings?)
			...
			*/
			setInitialTimelines : function () {
				var me = this;
				var tid = settings.initial_timeline_id;
				if (tid) {
		...
		****************************************
		*/
		function TimegliderTimelineView (_model, settings) {
			...
			$(TICKS)
				...
				.delegate(".ev", "click", function () { 
					// EVENT ON-CLICK !!!!!!
					var eid = $(this).attr("id"); 
					// output("click, id:" + eid, "note");
					me.eventModal(eid);
				})	
				.delegate(".ev", "hover", function () { 
					var eid = $(this).attr("id"); 
		...
		/*
		empty timeline class, awaiting flesh */
		function TimegliderTimeline(obj) {
			return obj;
		};

		TimegliderTimeline.prototype = {
				/* methods 
				chew
				getTitle
				getBounds
	    ...
	    this.load(MAIN_TEMPLATE, {}, function() {
			var settingsCheck = validateSettings(settings);
			if (settingsCheck == "") {
				$(CONTAINER).css({"backgroundColor":settings.backgroundColor});
				
				MODEL = new TimegliderMediator();
				MODEL.setFocusDate(tgDate.makeDateObject(settings.initial_focus));

				var timelineView = new TimegliderTimelineView(MODEL, settings);
				
				// load timelines
				MODEL.loadTimelineData(settings.data_source);
				
				// close the timelines menu
				timelineView.toggleMenu();
				
			} else {
				alert("There was a problem with the settings for the Timeglider plugin." + settingsCheck);
			}
		}); // end load callback
