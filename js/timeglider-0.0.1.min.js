var TimegliderInfo="Min version 0.0.1 on Tuesday Feb 8, 2011";window.debug=function(){function g(L){if(m&&(r||!o||!o.log))m.apply(x,L)}for(var x=this,p=Array.prototype.slice,o=x.console,C={},m,r,s=9,y=["error","warn","info","debug","log"],D="assert clear count dir dirxml exception group groupCollapsed groupEnd profile profileEnd table time timeEnd trace".split(" "),v=D.length,F=[];--v>=0;)(function(L){C[L]=function(){s!==0&&o&&o[L]&&o[L].apply(o,arguments)}})(D[v]);for(v=y.length;--v>=0;)(function(L,Q){C[Q]=function(){var S=p.call(arguments),X=[Q].concat(S);
F.push(X);g(X);if(o&&(s>0?s>L:y.length+s<=L))o.firebug?o[Q].apply(x,S):o[Q]?o[Q](S):o.log(S)}})(v,y[v]);C.setLevel=function(L){s=typeof L==="number"?L:9};C.setCallback=function(){var L=p.call(arguments),Q=F.length,S=Q;m=L.shift()||null;r=typeof L[0]==="boolean"?L.shift():false;for(S-=typeof L[0]==="number"?L.shift():Q;S<Q;)g(F[S++])};return C}();(function(g){function x(o){var C=o||window.event,m=[].slice.call(arguments,1),r=0,s=0,y=0;o=g.event.fix(C);o.type="mousewheel";if(o.wheelDelta)r=o.wheelDelta/120;if(o.detail)r=-o.detail/3;y=r;if(C.axis!==undefined&&C.axis===C.HORIZONTAL_AXIS){y=0;s=-1*r}if(C.wheelDeltaY!==undefined)y=C.wheelDeltaY/120;if(C.wheelDeltaX!==undefined)s=-1*C.wheelDeltaX/120;m.unshift(o,r,s,y);return g.event.handle.apply(this,m)}var p=["DOMMouseScroll","mousewheel"];g.event.special.mousewheel={setup:function(){if(this.addEventListener)for(var o=
...
c(h[d]):h[d];j[n]>=1E3&&delete h[j.shift()];j[Y](d);h[d]=a[aa](b,f);return c?c(h[d]):h[d]}return e}function o(){return this.x+fa+this.y}function C(a){return function(b,c,e,f){var d={back:a};g.is(e,"function")?f=e:d.rot=e;b&&b.constructor==ea&&(b=b.attrs.path);b&&(d.along=b);return this.animate(d,c,f)}}function m(a,b,c,e,f,d){function h(I,W){var K,E,P,ga;P=I;for(E=0;E<8;E++){ga=((l*P+k)*P+j)*P-I;if(J(ga)<W)return P;K=(3*l*P+2*k)*P+j;if(J(K)<1.0E-6)break;P-=ga/K}K=0;E=1;P=I;if(P<K)return K;if(P>E)return E;
for(;K<E;){ga=((l*P+k)*P+j)*P;if(J(ga-I)<W)break;I>ga?K=P:E=P;P=(E-K)/2+K}return P}var j=3*b,k=3*(e-b)-j,l=1-j-k,u=3*c,z=3*(f-c)-u,A=1-u-z;return function(I,W){var K=h(I,W);return((A*K+z)*K+u)*K}(a,1/(200*d))}g.version="1.5.2";var r=/[, ]+/,s={circle:1,rect:1,path:1,ellipse:1,text:1,image:1},y=/\{(\d+)\}/g,D="prototype",v="hasOwnProperty",F=document,L=window,Q={was:Object[D][v].call(L,"Raphael"),is:L.Raphael},S=function(){this.customAttributes={}},X,aa="apply",ra="createTouch"in F,fa=" ",Z=String,
...
zb=/([achlmqstvz])[\s,]*((-?\d*\.?\d*(?:e[-+]?\d+)?\s*,?\s*)+)/ig,Ab=/(-?\d*\.?\d*(?:e[-+]?\d+)?)\s*,?\s*/ig,$a=/^r(?:\(([^,]+?)\s*,\s*([^\)]+?)\))?/,Bb=function(a,b){return a.key-b.key};g.type=L.SVGAngle||F.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#BasicStructure","1.1")?"SVG":"VML";if(g.type=="VML"){var sa=F.createElement("div");sa.innerHTML='<v:shape adj="1"/>';sa=sa.firstChild;sa.style.behavior="url(#default#VML)";if(!(sa&&typeof sa.adj=="object"))return g.type=null;sa=null}g.svg=
...
180/O%360};g.snapTo=function(a,b,c){c=g.is(c,"finite")?c:10;if(g.is(a,R))for(var e=a.length;e--;){if(J(a[e]-b)<=c)return a[e]}else{a=+a;e=b%a;if(e<c)return b-e;if(e>a-c)return b-e+a}return b};g.setWindow=function(a){L=a;F=L.document};var Ga=function(a){if(g.vml){var b=/^\s+|\s+$/g,c;try{var e=new ActiveXObject("htmlfile");e.write("<body>");e.close();c=e.body}catch(f){c=createPopup().document.body}var d=c.createTextRange();Ga=p(function(j){try{c.style.color=Z(j).replace(b,"");var k=d.queryCommandValue("ForeColor");
return"#"+("000000"+((k&255)<<16|k&65280|(k&16711680)>>>16)[ba](16)).slice(-6)}catch(l){return"none"}})}else{var h=F.createElement("i");h.title="Rapha\u00ebl Colour Picker";h.style.display="none";F.body.appendChild(h);Ga=p(function(j){h.style.color=j;return F.defaultView.getComputedStyle(h,"").getPropertyValue("color")})}return Ga(a)},ab=function(){return"hsb("+[this.h,this.s,this.b]+")"},Cb=function(){return"hsl("+[this.h,this.s,this.l]+")"},Db=function(){return this.hex};g.hsb2rgb=function(a,b,
...
b[h][b[h][n]-1]}}b[ba]=g._path2string;return b},null,Ha),bb=function(a,b,c,e,f,d){var h=1/3,j=2/3;return[h*a+j*c,h*b+j*e,h*f+j*c,h*d+j*e,f,d]},cb=function(a,b,c,e,f,d,h,j,k,l){var u=O*120/180,z=O/180*(+f||0),A=[],I,W=p(function(ka,la,ia){var za=ka*B.cos(ia)-la*B.sin(ia);ka=ka*B.sin(ia)+la*B.cos(ia);return{x:za,y:ka}});if(l){P=l[0];I=l[1];d=l[2];E=l[3]}else{I=W(a,b,-z);a=I.x;b=I.y;I=W(j,k,-z);j=I.x;k=I.y;B.cos(O/180*f);B.sin(O/180*f);I=(a-j)/2;P=(b-k)/2;E=I*I/(c*c)+P*P/(e*e);if(E>1){E=B.sqrt(E);c*=
E;e*=E}E=c*c;var K=e*e;E=(d==h?-1:1)*B.sqrt(J((E*K-E*P*P-K*I*I)/(E*P*P+K*I*I)));d=E*c*P/e+(a+j)/2;var E=E*-e*I/c+(b+k)/2,P=B.asin(((b-E)/e).toFixed(9));I=B.asin(((k-E)/e).toFixed(9));P=a<d?O-P:P;I=j<d?O-I:I;P<0&&(P=O*2+P);I<0&&(I=O*2+I);h&&P>I&&(P-=O*2);!h&&I>P&&(I-=O*2)}if(J(I-P)>u){A=I;K=j;var ga=k;I=P+u*(h&&I>P?1:-1);j=d+c*B.cos(I);k=E+e*B.sin(I);A=cb(j,k,c,e,f,0,h,K,ga,[I,A,d,E])}d=I-P;f=B.cos(P);u=B.sin(P);h=B.cos(I);I=B.sin(I);d=B.tan(d/4);c=4/3*c*d;d*=4/3*e;e=[a,b];a=[a+c*u,b-d*f];b=[j+c*I,
...
e=b&&Ba(b),f={x:0,y:0,bx:0,by:0,X:0,Y:0,qx:null,qy:null},d={x:0,y:0,bx:0,by:0,X:0,Y:0,qx:null,qy:null},h=function(K,E){var P,ga;if(!K)return["C",E.x,E.y,E.x,E.y,E.x,E.y];!(K[0]in{T:1,Q:1})&&(E.qx=E.qy=null);switch(K[0]){case "M":E.X=K[1];E.Y=K[2];break;case "A":K=["C"].concat(cb[aa](0,[E.x,E.y].concat(K.slice(1))));break;case "S":P=E.x+(E.x-(E.bx||E.x));ga=E.y+(E.y-(E.by||E.y));K=["C",P,ga].concat(K.slice(1));break;case "T":E.qx=E.x+(E.x-(E.qx||E.x));E.qy=E.y+(E.y-(E.qy||E.y));K=["C"].concat(bb(E.x,
E.y,E.qx,E.qy,K[1],K[2]));break;case "Q":E.qx=K[1];E.qy=K[2];K=["C"].concat(bb(E.x,E.y,K[1],K[2],K[3],K[4]));break;case "L":K=["C"].concat([E.x,E.y,K[1],K[2],K[1],K[2]]);break;case "H":K=["C"].concat([E.x,E.y,K[1],E.y,K[1],E.y]);break;case "V":K=["C"].concat([E.x,E.y,E.x,K[1],E.x,K[1]]);break;case "Z":K=["C"].concat([E.x,E.y,E.X,E.Y,E.X,E.Y])}return K},j=function(K,E){if(K[E][n]>7){K[E].shift();for(var P=K[E];P[n];)K.splice(E++,0,["C"].concat(P.splice(0,6)));K.splice(E,1);u=G(c[n],e&&e[n]||0)}},k=
function(K,E,P,ga,ka){if(K&&E&&K[ka][0]=="M"&&E[ka][0]!="M"){E.splice(ka,0,["M",ga.x,ga.y]);P.bx=0;P.by=0;P.x=K[ka][1];P.y=K[ka][2];u=G(c[n],e&&e[n]||0)}},l=0,u=G(c[n],e&&e[n]||0);for(;l<u;l++){c[l]=h(c[l],f);j(c,l);e&&(e[l]=h(e[l],d));e&&j(e,l);k(c,e,f,d,l);k(e,c,d,f,l);var z=c[l],A=e&&e[l],I=z[n],W=e&&A[n];f.x=z[I-2];f.y=z[I-1];f.bx=M(z[I-4])||f.x;f.by=M(z[I-3])||f.y;d.bx=e&&(M(A[W-4])||d.x);d.by=e&&(M(A[W-3])||d.y);d.x=e&&A[W-2];d.y=e&&A[W-1]}return e?[c,e]:c},null,Ha),db=p(function(a){for(var b=
[],c=0,e=a[n];c<e;c++){var f={},d=a[c].match(/^([^:]*):?([\d\.]*)/);f.color=g.getRGB(d[1]);if(f.color.error)return null;f.color=f.color.hex;d[2]&&(f.offset=d[2]+"%");b[Y](f)}c=1;for(e=b[n]-1;c<e;c++)if(!b[c].offset){a=M(b[c-1].offset||0);d=0;for(f=c+1;f<e;f++)if(b[f].offset){d=b[f].offset;break}if(!d){d=100;f=e}d=M(d);for(d=(d-a)/(f-c+1);c<f;c++){a+=d;b[c].offset=a+"%"}}return b}),eb=function(a,b,c,e){if(g.is(a,"string")||g.is(a,"object")){a=g.is(a,"string")?F.getElementById(a):a;if(a.tagName)return b==
...
a+"\u201d of removed object");}};g.pathToRelative=Ra;if(g.svg){X.svgns="http://www.w3.org/2000/svg";X.xlink="http://www.w3.org/1999/xlink";T=function(a){return+a+(~~a===a)*0.5};var U=function(a,b){if(b)for(var c in b)b[v](c)&&a.setAttribute(c,Z(b[c]));else{a=F.createElementNS(X.svgns,a);a.style.webkitTapHighlightColor="rgba(0,0,0,0)";return a}};g[ba]=function(){return"Your browser supports SVG.\nYou are running Rapha\u00ebl "+this.version};var kb=function(a,b){var c=U("path");b.canvas&&b.canvas.appendChild(c);
...
F.getElementById(e.getAttribute("fill").replace(/^url\(#|\)$/g,""))){u=u.getElementsByTagName("stop");u[u[n]-1].setAttribute("stop-opacity",l)}break}default:k=="font-size"&&(l=ja(l,10)+"px");u=k.replace(/(\-.)/g,function(W){return xa.call(W.substring(1))});e.style[u]=l;e.setAttribute(k,l)}}Fb(a,b);j?a.rotate(j.join(fa)):M(d)&&a.rotate(d,true)},Fb=function(a,b){if(!(a.type!="text"||!(b[v]("text")||b[v]("font")||b[v]("font-size")||b[v]("x")||b[v]("y")))){var c=a.attrs,e=a.node,f=e.firstChild?ja(F.defaultView.getComputedStyle(e.firstChild,
"").getPropertyValue("font-size"),10):10;if(b[v]("text")){for(c.text=b.text;e.firstChild;)e.removeChild(e.firstChild);for(var d=Z(b.text).split("\n"),h=0,j=d[n];h<j;h++)if(d[h]){var k=U("tspan");h&&U(k,{dy:f*1.2,x:c.x});k.appendChild(F.createTextNode(d[h]));e.appendChild(k)}}else{d=e.getElementsByTagName("tspan");h=0;for(j=d[n];h<j;h++)h&&U(d[h],{dy:f*1.2,x:c.x})}U(e,{y:c.y});f=a.getBBox();(f=c.y-(f.y+f.height/2))&&g.is(f,"finite")&&U(e,{y:c.y+f})}},ea=function(a,b){this[0]=a;this.id=g._oid++;this.node=
...
return this},Oa=function(){var a=eb[aa](0,arguments),b=a&&a.container,c=a.x,e=a.y,f=a.width;a=a.height;if(!b)throw Error("SVG container not found.");var d=U("svg");c=c||0;e=e||0;f=f||512;a=a||342;U(d,{xmlns:"http://www.w3.org/2000/svg",version:1.1,width:f,height:a});if(b==1){d.style.cssText="position:absolute;left:"+c+"px;top:"+e+"px";F.body.appendChild(d)}else b.firstChild?b.insertBefore(d,b.firstChild):b.appendChild(d);b=new S;b.width=f;b.height=a;b.canvas=d;Sa.call(b,b,g.fn);b.clear();return b};
X.clear=function(){for(var a=this.canvas;a.firstChild;)a.removeChild(a.firstChild);this.bottom=this.top=null;(this.desc=U("desc")).appendChild(F.createTextNode("Created with Rapha\u00ebl"));a.appendChild(this.desc);a.appendChild(this.defs=U("defs"))};X.remove=function(){this.canvas.parentNode&&this.canvas.parentNode.removeChild(this.canvas);for(var a in this)this[a]=jb(a)}}if(g.vml){var sb={M:"m",L:"l",C:"c",Z:"x",m:"t",l:"r",c:"v",z:"x"},Gb=/([clmz]),?([^clmz]*)/gi,Hb=/ progid:\S+Blur\([^\)]+\)/g,
...
La.call(a,d[0],d[1]);if(a._.rt.cx!=null){a._.rt.cx+=+d[0];a._.rt.cy+=+d[1];a.setBox(a.attrs,d[0],d[1])}}if(b.scale){d=Z(b.scale).split(r);a.scale(+d[0]||1,+d[1]||+d[0]||1,+d[2]||null,+d[3]||null)}if("clip-rect"in b){d=Z(b["clip-rect"]).split(r);if(d[n]==4){d[2]=+d[2]+ +d[0];d[3]=+d[3]+ +d[1];h=c.clipRect||F.createElement("div");var j=h.style,k=c.parentNode;j.clip=g.format("rect({1}px {2}px {3}px {0}px)",d);if(!c.clipRect){j.position="absolute";j.top=0;j.left=0;j.width=a.paper.width+"px";j.height=
...
a==+a&&(a+="px");b==+b&&(b+="px");c.width=a;c.height=b;c.clip="rect(0 "+a+" "+b+" 0)";return this};var ma;F.createStyleSheet().addRule(".rvml","behavior:url(#default#VML)");try{!F.namespaces.rvml&&F.namespaces.add("rvml","urn:schemas-microsoft-com:vml");ma=function(a){return F.createElement("<rvml:"+a+' class="rvml">')}}catch(Qb){ma=function(a){return F.createElement("<"+a+' xmlns="urn:schemas-microsoft.com:vml" class="rvml">')}}Oa=function(){var a=eb[aa](0,arguments),b=a.container,c=a.height,e=a.width,
f=a.x;a=a.y;if(!b)throw Error("VML container not found.");var d=new S,h=d.canvas=F.createElement("div"),j=h.style;f=f||0;a=a||0;e=e||512;c=c||342;e==+e&&(e+="px");c==+c&&(c+="px");d.width=1E3;d.height=1E3;d.coordsize=1E4+fa+1E4;d.coordorigin="0 0";d.span=F.createElement("span");d.span.style.cssText="position:absolute;left:-9999em;top:-9999em;padding:0;margin:0;line-height:1;display:inline;";h.appendChild(d.span);j.cssText=g.format("top:0;left:0;width:{0};height:{1};display:inline-block;position:relative;clip:rect(0 {0} {1} 0);overflow:hidden",
e,c);if(b==1){F.body.appendChild(h);j.left=f+"px";j.top=a+"px";j.position="absolute"}else b.firstChild?b.insertBefore(h,b.firstChild):b.appendChild(h);Sa.call(d,d,g.fn);return d};X.clear=function(){this.canvas.innerHTML="";this.span=F.createElement("span");this.span.style.cssText="position:absolute;left:-9999em;top:-9999em;padding:0;margin:0;line-height:1;display:inline;";this.canvas.appendChild(this.span);this.bottom=this.top=null};X.remove=function(){this.canvas.parentNode.removeChild(this.canvas);
for(var a in this)this[a]=jb(a);return true}}sa=navigator.userAgent.match(/Version\\x2f(.*?)\s/);navigator.vendor=="Apple Computer, Inc."&&(sa&&sa[1]<4||navigator.platform.slice(0,2)=="iP")?X.safari=function(){var a=this.rect(-99,-99,this.width+99,this.height+99).attr({stroke:"none"});L.setTimeout(function(){a.remove()})}:X.safari=function(){};var Kb=function(){this.returnValue=false},Lb=function(){return this.originalEvent.preventDefault()},Mb=function(){this.cancelBubble=true},Nb=function(){return this.originalEvent.stopPropagation()},
Ob=function(){if(F.addEventListener)return function(a,b,c,e){var f=ra&&qa[b]?qa[b]:b,d=function(h){if(ra&&qa[v](b))for(var j=0,k=h.targetTouches&&h.targetTouches.length;j<k;j++)if(h.targetTouches[j].target==a){k=h;h=h.targetTouches[j];h.originalEvent=k;h.preventDefault=Lb;h.stopPropagation=Nb;break}return c.call(e,h)};a.addEventListener(f,d,false);return function(){a.removeEventListener(f,d,false);return true}};if(F.attachEvent)return function(a,b,c,e){var f=function(d){d=d||L.event;d.preventDefault=
d.preventDefault||Kb;d.stopPropagation=d.stopPropagation||Mb;return c.call(e,d)};a.attachEvent("on"+b,f);return function(){a.detachEvent("on"+b,f);return true}}}(),ta=[],Ua=function(a){for(var b=a.clientX,c=a.clientY,e=F.documentElement.scrollTop||F.body.scrollTop,f=F.documentElement.scrollLeft||F.body.scrollLeft,d,h=ta.length;h--;){d=ta[h];if(ra)for(var j=a.touches.length,k;j--;){k=a.touches[j];if(k.identifier==d.el._drag.id){b=k.clientX;c=k.clientY;(a.originalEvent?a.originalEvent:a).preventDefault();
break}}else a.preventDefault();b+=f;c+=e;d.move&&d.move.call(d.move_scope||d.el,b-d.el._drag.x,c-d.el._drag.y,b,c,a)}},Va=function(a){g.unmousemove(Ua).unmouseup(Va);for(var b=ta.length,c;b--;){c=ta[b];c.el._drag={};c.end&&c.end.call(c.end_scope||c.start_scope||c.move_scope||c.el,a)}ta=[]};for(sa=va[n];sa--;)(function(a){g[a]=ea[D][a]=function(b,c){if(g.is(b,"function")){this.events=this.events||[];this.events.push({name:a,f:b,unbind:Ob(this.shape||this.node||F,a,b,c||this)})}return this};g["un"+
a]=ea[D]["un"+a]=function(b){for(var c=this.events,e=c[n];e--;)if(c[e].name==a&&c[e].f==b){c[e].unbind();c.splice(e,1);!c.length&&delete this.events;break}return this}})(va[sa]);ca.hover=function(a,b,c,e){return this.mouseover(a,c).mouseout(b,e||c)};ca.unhover=function(a,b){return this.unmouseover(a).unmouseout(b)};ca.drag=function(a,b,c,e,f,d){this._drag={};this.mousedown(function(h){(h.originalEvent||h).preventDefault();var j=F.documentElement.scrollTop||F.body.scrollTop,k=F.documentElement.scrollLeft||
F.body.scrollLeft;this._drag.x=h.clientX+k;this._drag.y=h.clientY+j;this._drag.id=h.identifier;b&&b.call(f||e||this,h.clientX+k,h.clientY+j,h);!ta.length&&g.mousemove(Ua).mouseup(Va);ta.push({el:this,move:a,end:c,move_scope:e,start_scope:f,end_scope:d})});return this};ca.undrag=function(a,b,c){for(b=ta.length;b--;)ta[b].el==this&&ta[b].move==a&&ta[b].end==c&&ta.splice(b++,1);!ta.length&&g.unmousemove(Ua).unmouseup(Va)};X.circle=function(a,b,c){return mb(this,a||0,b||0,c||0)};X.rect=function(a,b,c,
...
g;ca.resetScale=function(){if(this.removed)return this;this._.sx=1;this._.sy=1;this.attrs.scale="1 1"};ca.scale=function(a,b,c,e){if(this.removed)return this;if(a==null&&b==null)return{x:this._.sx,y:this._.sy,toString:o};b=b||a;!+b&&(b=a);var f,d,h=this.attrs;if(a!=0){var j=this.getBBox(),k=j.x+j.width/2,l=j.y+j.height/2;f=J(a/this._.sx);d=J(b/this._.sy);c=+c||c==0?c:k;e=+e||e==0?e:l;var u=this._.sx>0,z=this._.sy>0;j=~~(a/J(a));var A=~~(b/J(b)),I=f*j,W=d*A,K=this.node.style;k=c+J(k-c)*I*(k>c==u?1:
-1);l=e+J(l-e)*W*(l>e==z?1:-1);var E=a*j>b*A?d:f;switch(this.type){case "rect":case "image":var P=h.width*f,ga=h.height*d;this.attr({height:ga,r:h.r*E,width:P,x:k-P/2,y:l-ga/2});break;case "circle":case "ellipse":this.attr({rx:h.rx*f,ry:h.ry*d,r:h.r*E,cx:k,cy:l});break;case "text":this.attr({x:k,y:l});break;case "path":E=Ra(h.path);var ka=true;u=u?I:f;z=z?W:d;W=0;for(I=E[n];W<I;W++){var la=E[W],ia=xa.call(la[0]);if(!(ia=="M"&&ka)){ka=false;if(ia=="A"){la[E[W][n]-2]*=u;la[E[W][n]-1]*=z;la[1]*=f;la[2]*=
d;la[5]=+(j+A?!!+la[5]:!+la[5])}else if(ia=="H"){ia=1;for(var za=la[n];ia<za;ia++)la[ia]*=u}else if(ia=="V"){ia=1;for(za=la[n];ia<za;ia++)la[ia]*=z}else{ia=1;for(za=la[n];ia<za;ia++)la[ia]*=ia%2?u:z}}}d=Ja(E);f=k-d.x-d.width/2;d=l-d.y-d.height/2;E[0][1]+=f;E[0][2]+=d;this.attr({path:E})}if(this.type in{text:1,image:1}&&(j!=1||A!=1))if(this.transformations){this.transformations[2]="scale(".concat(j,",",A,")");this.node.setAttribute("transform",this.transformations[q](fa));f=j==-1?-h.x-(P||0):h.x;d=
...
if(e<f){e=d(e/f);for(var A in h)if(h[v](A)){switch(Pa[A]){case "along":z=e*f*j[A];k.back&&(z=k.len-z);d=Na(k[A],z);u.translate(j.sx-j.x||0,j.sy-j.y||0);j.x=d.x;j.y=d.y;u.translate(d.x-j.sx,d.y-j.sy);k.rot&&u.rotate(j.r+d.alpha,d.x,d.y);break;case V:z=+h[A]+e*f*j[A];break;case "colour":z="rgb("+[Ya(T(h[A].r+e*f*j[A].r)),Ya(T(h[A].g+e*f*j[A].g)),Ya(T(h[A].b+e*f*j[A].b))][q](",")+")";break;case "path":z=[];d=0;for(var I=h[A][n];d<I;d++){z[d]=[h[A][d][0]];for(var W=1,K=h[A][d][n];W<K;W++)z[d][W]=+h[A][d][W]+
...
function F(){return/\/|dddd|ddd|dd|d|MMMM|MMM|MM|M|yyyy|yy|y|hh|h|HH|H|mm|m|ss|s|tt|t|fff|ff|f|zzz|zz|z|gg|g/g}function L(q,n,w){q=g(q);w=w.calendar;a:{var B=w._parseRegExp;if(B){var G=B[n];if(G){n=G;break a}}else w._parseRegExp=B={};G=v(w,n).replace(/([\^\$\.\*\+\?\|\[\]\(\)\{\}])/g,"\\\\$1");for(var N=["^"],J=[],H=0,O=0,V=F(),R;(R=V.exec(G))!==null;){var ba=G.slice(H,R.index);H=V.lastIndex;O+=D(ba,N);if(O%2)N.push(R[0]);else{ba=R[0];var wa=ba.length;switch(ba){case "dddd":case "ddd":case "MMMM":case "MMM":case "gg":case "g":ba=
...
J,O));w=w.toLocaleString()}else w=q.toString();return w}O=J.eras;var V=n==="s";n=v(J,n);w=[];var R,ba=["0","00","000"],wa,Y,ha=/([^d]|^)(d|dd)([^d]|$)/g,na=0,da=F(),oa;if(!V&&H)oa=H.fromGregorian(q);for(;;){R=da.lastIndex;H=da.exec(n);R=n.slice(R,H?H.index:n.length);na+=D(R,w);if(!H)break;if(na%2)w.push(H[0]);else{R=H[0];H=R.length;switch(R){case "ddd":case "dddd":names=H===3?J.days.namesAbbr:J.days.names;w.push(names[q.getDay()]);break;case "d":case "dd":wa=true;w.push(B(N(q,2),H));break;case "MMM":case "MMMM":R=
...
"Sat"],dayNamesLet:["S","M","T","W","T","F","S"],units:["da","mo","ye","de","ce","thou"],isLeapYear:function(g){return g%400==0?true:g%100==0?false:g%4==0?true:false},boil:function(g){return parseInt(g,10)},getTimeUnitSerial:function(g,x){switch(x){case "ye":return g.ye;case "mo":return this.getMonthNum(g);case "da":return this.getRataDie(g);case "de":return Math.ceil(g.ye/10);case "ce":return Math.ceil(g.ye/100);case "thou":return Math.ceil(g.ye/1E3);case "tenthou":return Math.ceil(g.ye/1E4);case "hundredthou":return Math.ceil(g.ye/
...
x={ye:this.boil(g[0]),mo:this.boil(g[1]),da:this.boil(g[2]),ho:this.boil(g[3]),mi:this.boil(g[4]),se:this.boil(g[5]),bce:x};x.rd=this.getRataDie(x);x.sec=this.getSec(x);return x},getMonthDays:function(g,x){return this.isLeapYear(x)==true&&g==2?29:this.monthsDayNums[g]},isValidDate:function(g,x,p){var o=this.getMonthDays(x,g);if(p>o||p<=0)return false;if(x>12||x<0)return false;if(g==0)return false;if(typeof g!="number"||String(g).match(/([0-9]+)/)==false)return false;return true},formatFocusDate:function(g){return g.ye+
...
181;break;case 8:p=212;break;case 9:p=243;break;case 10:p=273;break;case 11:p=304;break;case 12:p=334}if(g>2&&this.isLeapYear(x))p+=1;return p},getToday:function(){var g=jQuery.format(new Date,"S");g=g.replace("T"," ");debug.log("df:"+g);return g}};String.prototype.trim=function(){return this.replace(/^\s+|\s+$/g,"")};String.prototype.ltrim=function(){return this.replace(/^\s+/,"")};String.prototype.rtrim=function(){return this.replace(/\s+$/,"")};function output(g,x){$("#"+x).text(g)};(function(g){g.levelHeight=8;var x=g.levelHeight;g.TGOrg=function(){this.blocks=[];this.ids=[];this.vis=[];this.tree=[];var p=this;this.addBlock=function(m,r){m.right=m.left+m.width;m.bottom=m.top+m.height;m.tickScope=r;p.blocks.push(m)};this.getBorg=function(){return this};this.getBlocks=function(){return this.blocks};this.getHTML=function(m){if(m=="sweep"){p.tree=[];for(var r=0;r<300;r++)p.tree[r]=[];this.vis=[]}this.blocks.sort(o);for(var s,y="",D="",v={},F=this.blocks.length,L=0,Q=0;Q<F;Q++){v=
this.blocks[Q];if(v.tickScope==m)if(jQuery.inArray(v.id,this.vis)==-1){this.vis.push(v.id);L=0;if(v.image){if(v.image_class=="layout")L=v.image_size.height+4;y="<div class='timeglider-event-image-"+v.image_class+"'><img src='"+v.image+"'></div>"}else y="";C(v,0);v.fontsize<10?v.opacity=v.fontsize/10:v.opacity=1;if(v.span==true){r="timeglider-event-spanning";s="<div class='timeglider-event-spanner' style='height:"+v.fontsize+"px;width:"+v.spanwidth+"px'></div>"}else s=r="";D+="<div class='timeglider-timeline-event "+
r+"' id='ev_"+v.id+"' style='width:"+v.width+"px;height:"+v.height+"px;left:"+v.left+"px;opacity:"+v.opacity+";top:"+v.top+"px;font-size:"+v.fontsize+"px;'><img class='timeglider-event-icon' src='"+v.icon+"' style='height:"+v.fontsize+"px;left:-"+(v.fontsize+2)+"px; top:"+L+"px'>"+y+s+"<div class='timeglider-event-title' style='top:"+L+"px'>"+v.title+"</div></div>"}}return D};var o=function(m,r){var s=r.importance,y=m.importance;return s<y?-1:s>y?1:0},C=function(m,r){var s=false,y=p.tree,D=y[r],v=
r+1,F=false,L=2;bump_ht=x;if(D!=undefined){for(L=0;L<D.length;L++){a:{s=D[L];if(!(m.left>s.right||m.right<s.left))if(m.left>=s.left&&m.left<=s.right||m.right>=s.left&&m.right<=s.right||m.right>=s.right&&m.left<=s.left||m.right<=s.right&&m.left>=s.left)if(m.bottom<=s.bottom&&m.bottom>=s.top||m.top<=s.bottom&&m.top>=s.top||m.top==s.bottom&&m.top==s.top){s=true;break a}s=false}if(s==true){m.top-=bump_ht;m.bottom-=bump_ht;C(m,v);F=true;break}}if(F==false){m.top-=m.fontsize;D.push(m);L=Math.ceil(m.height/
x);D=0;for(D=1;D<=L;D++){v=r+D;y[v].push(m)}}}}}})(timeglider);(function(g){var x=g.TGDate,p,o=jQuery;g.TimegliderTimelineView=function(m,r){var s=m.options,y="#"+m._id,D=o(y).height(),v=this;p=r;this._views={PLACE:y,CONTAINER:y+" .timeglider-container",MENU:y+" .timeglider-timeline-menu",MENU_UL:y+" .timeglider-timeline-menu ul",MENU_HANDLE:y+" .timeglider-timeline-menu-handle",SLIDER_CONTAINER:y+" .timeglider-slider-container",SLIDER:y+" .timeglider-slider",TRUCK:y+" .timeglider-truck",CENTERLINE:y+" .timeglider-centerline",TICKS:y+" .timeglider-ticks",HANDLE:y+
...
function(){});o.subscribe("mediator.focusDateChange",function(){});o.subscribe("mediator.timelineListChangeSignal",function(){debug.log("timelineListChangeSignal...");v.adjustTimelineTitleStyles();v.buildTimelineMenu()});o.subscribe("mediator.activeTimelinesChange",function(){o(v._views.MENU_UL+" li").each(function(){var F=o(this).attr("id");o.inArray(F,p._activeTimelines)!=-1?o(this).addClass("activeTimeline"):o(this).removeClass("activeTimeline")})});o(this._views.FILTER_BT).click(function(){o(this);
var F=v._views.FILTER_BOX;if(v.filterBoxActivated==false){o(F).append(C);v.filterBoxActivated=true;var L=o(F+" .timeglider-filter-apply"),Q=o(F+" .timeglider-filter-close"),S=o(F+" .timeglider-filter-clear"),X="",aa="";L.click(function(){X=o(F+" .timeglider-filter-include").val();aa=o(F+" .timeglider-filter-exclude").val();p.setFilterObject({include:X,exclude:aa});o(F).toggleClass("box-visible")});Q.click(function(){o(F).toggleClass("box-visible")});S.click(function(){p.setFilterObject({include:"",
exclude:""});o(F+" .timeglider-filter-include").val("");o(F+" .timeglider-filter-exclude").val("");o(F).toggleClass("box-visible")})}o(v._views.FILTER_BOX).toggleClass("box-visible").css("z-index",v.ztop++)});o.subscribe("mediator.filterObjectChange",function(){});o(this._views.TRUCK).dblclick(function(F){var L=F.pageX-v.dimensions.container.offset.left-v.dimensions.container.width/2,Q=p.getZoomInfo().spp;F=p.getFocusDate().sec;L=x.getDateFromSec(Math.floor(F+L*Q));F=x.getDateFromSec(F);output("DBLCLICK:"+
F.mo+"-"+F.ye+" dblclick:"+L.mo+"-"+L.ye,"note")}).bind("mousewheel",function(F,L){var Q=Math.ceil(-1*L*3),S=p.getZoomLevel();p.setZoomLevel(S+Q);return false});o(this._views.TICKS).draggable({axis:"x",drag:function(){p.setTicksOffset(o(this).position().left)},stop:function(){v.resetTicksHandle();v.easeOutTicks();v.registerDragging()}}).delegate(".timeglider-timeline-event","click",function(){var F=o(this).attr("id");v.eventModal(F)}).delegate(".timeglider-timeline-event","hover",function(){var F=
o(this).attr("id");output("hover, title:"+p.eventPool[F].title,"note")}).delegate(".timeglider-event-collapsed","hover",function(){var F=o(this).attr("id");output("collapsed, title:"+p.eventPool[F].title,"note")});o(".timeglider-ev-modal .close-button").live("click",function(){o(this).parent().remove()});o(".timeglider-ev-video-modal .close-button").live("click",function(){o(this).parent().remove()});o(this._views.MENU_HANDLE).click(function(){v.toggleMenu()});s.show_centerline===true?o(this._views.CENTERLINE).css({height:v.dimensions.container.height,
left:v.dimensions.container.centerx}):o(this._views.CENTERLINE).css({display:"none"})};g.TimegliderTimelineView.prototype={getWidgetDimensions:function(){var m=o(this._views.CONTAINER),r=m.width(),s=Math.floor(r/2)+1,y=m.height(),D=Math.floor(y/2),v=m.position().left;m=m.offset();r={width:r,height:y,centerx:s,centery:D,left:v,offset:m};s={height:o(this._views.CONTAINER+" .timeglider-footer").height()};return{container:r,tick:{top:y-s.height-30},footer:s}},scaleToImportance:function(m,r){return m/
r},invSliderVal:function(m){return Math.abs(m-101)},registerTitles:function(){var m,r,s,y,D,v,F,L,Q,S,X,aa=o(this._views.CONTAINER).offset().left;o(".timeglider-event-spanning").each(function(){m=o(this).offset().left-aa;r=o(this).outerWidth();F=o(".timeglider-event-title",this);s=F.outerWidth()+5;y=F.siblings(".timeglider-event-spanner").outerWidth();y>s&&m<0&&Math.abs(m)<r-s&&F.css({marginLeft:-1*m+5})});o(".tg-timeline-envelope").each(function(){L=o(this).offset().left-aa;Q=o(".titleBar",this);
S=o(".titleBar .timeline-title",this);D=Q.position().left;X=D+L;Q.outerWidth();s=Q.outerWidth();v=-1*X;X<0&&S.css({marginLeft:v+5})})},registerDragging:function(){var m=p._startSec,r=o(this._views.TICKS).position().left,s=p.getZoomInfo().spp;m=x.getDateFromSec(m-r*s);output("ticks x:"+r,"tickpos");output("FD: "+x.formatFocusDate(m),"focusdate");p.setFocusDate(m)},adjustTimelineTitleStyles:function(){debug.log("modifyTimelineTitles!")},buildTimelineMenu:function(){debug.log("buildTimelineMenu!");var m,
r=p.timelinePool,s=0;o(this._views.MENU_UL+" li").remove();for(m in r){if(r.hasOwnProperty(m)){var y=r[m];o(this._views.MENU_UL).append("<li class = 'timelineList' id='"+m+"'>"+y.title+"</li>");o("li#"+m).click(function(){p.toggleTimeline(o(this).attr("id"))})}s++}},buildSlider:function(){var m=this,r=m.invSliderVal(p.getZoomLevel()),s=p.max_zoom,y=p.min_zoom;o(this._views.SLIDER).css("height",(1+s-y)*3).slider({steps:100,handle:o(".knob"),animate:500,orientation:"vertical",min:m.invSliderVal(s),
max:m.invSliderVal(y),value:r,start:function(){m.sliderActive=true},stop:function(){m.sliderActive=false},change:function(){},slide:function(D,v){p.setZoomLevel(m.invSliderVal(v.value))}})},clearTicks:function(){this.tickNum=0;o(this._views.TICKS).css("left",0).html("<div class='timeglider-handle'></div>")},castTicks:function(){p.getZoomLevel();var m=p.getFocusDate(),r=p.getZoomInfo().width;r=Math.ceil(this.dimensions.container.width/r)+4;var s="l";this.clearTicks();p.setTicksReady(false);this.addTick({type:"init",
focus_date:m});for(m=1;m<=r;m+=1){this.addTick({type:s});s=s=="l"?"r":"l"}p.setTicksReady(true)},addTick:function(m){var r=0,s=0,y=s=0,D=0,v=0,F="";v={};y={};var L={},Q={};v={};L=p.getZoomInfo().unit;Q=p.getZoomInfo().width;D=p.getFocusDate();y=parseInt(this.dimensions.tick.top);D=p.addToTicksArray({type:m.type,unit:L},D);if(L=="mo"){v=x.getMonthAdj(D,Q);Q=v.width;r=v.days}this.tickNum++;if(m.type=="init"){v=this.tickOffsetFromDate(p.getZoomInfo(),p.getFocusDate(),Q);this.leftside=s=Math.ceil(this.dimensions.container.centerx+
v);this.rightside=s+Q}else if(m.type=="l")this.leftside=s=Math.floor(this.leftside-Q);else if(m.type=="r"){s=Math.floor(this.rightside);this.rightside+=Q}p.getTickBySerial(D).width=Q;p.getTickBySerial(D).left=s;F=this._views.PLACE+"_"+L+"_"+D+"-"+this.tickNum;v=o("<div class='timeglider-tick' id='"+F+"'><div class='timeglider-tick-label' id='label'></div></div>").appendTo(this._views.TICKS);v.css({width:Q,left:s,top:y});y=this.getTickMarksInfo({unit:L,width:Q});y=r>0?r:y.tperu;s=Q/y;if(s>5){m=Raphael(F,
Q,30);var S="";for(r=0;r<y;++r){F=r*s;S+="M"+F+" 20 L"+F+" 30"}m.path(S).attr({stroke:"#333","stroke-width":1})}L={unit:L,width:Q,serial:D};Q=this.getDateLabelForTick(L);v.children("#label").text(Q);return L},getTickMarksInfo:function(m){switch(m.unit){case "da":m=24;break;case "mo":m=30;break;case "ye":m=12;break;default:m=10}return{tperu:m}},getDateLabelForTick:function(m){switch(m.unit){case "bill":if(m.serial==1)return"1";return m.serial-1+" bya";case "hundredmill":if(m.serial==1)return"1";return(m.serial-
1)*100+" mya";case "tenmill":if(m.serial==1)return"1";return(m.serial-1)*10+" mya";case "mill":if(m.serial==1)return"1";return m.serial-1+" mya";case "hundredthou":if(m.serial==1)return"1";return m.serial-1+"00,000";case "tenthou":if(m.serial==1)return"1";return m.serial-1+"0000";case "thou":if(m.serial==1)return"1";return m.serial-1+"000";case "ce":if(m.serial==1)return"1";return m.serial-1+"00";case "de":return(m.serial-1)*10+"s";case "ye":return m.serial;case "mo":m=x.getDateFromMonthNum(m.serial);
return x.monthNamesFull[m.mo]+", "+m.ye;case "da":m=x.getDateFromRD(m.serial);return x.monthNamesAbbr[m.mo]+" "+m.da+", "+m.ye;default:return m.unit+":"+m.serial+":"+m.width}},tickHangies:function(){var m=o(this._views.TICKS).position().left,r=this.rightside+m-this.dimensions.container.width;if(this.leftside+m>-100){m=this.addTick({type:"l"});this.appendTimelines(m)}else if(r<100){m=this.addTick({type:"r"});this.appendTimelines(m)}},tickOffsetFromDate:function(m,r,s){switch(m.unit){case "da":r=r.ho/
24+r.mi/1440;s*=r;break;case "mo":m=x.getMonthDays(r.mo,r.ye);r=(r.da-1)/m+r.ho/(24*m)+r.mi/(1440*m);s*=r;break;case "ye":r=((r.mo-1)*30+r.da)/365;s*=r;break;case "de":r=r.ye%10/10+r.mo/120;s*=r;break;case "ce":r=r.ye%100/100+r.mo/1200;s*=r;break;case "thou":r=r.ye%1E3/1E3;s*=r;break;case "tenthou":r=r.ye%1E4/1E4;s*=r;break;case "hundredthou":r=r.ye%1E5/1E5;s*=r;break;default:s=0}return-1*s},resetTicksHandle:function(){o(this._views.HANDLE).offset({left:o(this._views.CONTAINER).offset().left})},easeOutTicks:function(){Math.abs(this.dragSpeed)>
5&&output("ticks stopped!")},toggleMenu:function(){var m=o(this._views.MENU).width();if(p.timelineMenuOpen===false){o(this._views.MENU).animate({left:"+="+m},50);o(this._views.MENU_HANDLE).text("<<");p.timelineMenuOpen=true}else{o(this._views.MENU).animate({left:"-="+m},100);o(this._views.MENU_HANDLE).text("timelines >>");p.timelineMenuOpen=false}},getTimelineEventsByTick:function(m){var r=m.tick.unit,s=m.tick.serial;m=m.timeline.dateHash;return m[r][s]&&m[r][s].length>0?m[r][s]:0},setTimelineProp:function(m,
r,s){p.timelinePool[m][r]=s},passesFilters:function(m,r){var s=true,y="",D=[],v;y="";D=[];if(r<m.low_threshold||r>m.high_threshold)return false;if(y=p.filterObject.include){D=y.split(",");s=false;for(v=0;v<D.length;v++){y=RegExp(D[v].trim(),"i");if(m.title.match(y))s=true}}if(y=p.filterObject.exclude){D=y.split(",");for(v=0;v<D.length;v++){y=RegExp(D[v].trim(),"i");if(m.title.match(y))s=false}}return s},freshTimelines:function(){var m,r,s,y,D=p._activeTimelines,v=p._ticksArray,F="",L,Q=this,S="",
X=0,aa=Q.dimensions.container.centerx,ra=Q.dimensions.container.width,fa=p.getFocusDate().sec,Z=p.getZoomInfo().spp,va=p.getZoomInfo().level;m=[];var qa=[];ra=fa-Math.floor(Z*(ra/2));var q;s=0;for(var n=Q.dimensions.container.height,w=0;w<D.length;w++){r=p.timelinePool[D[w]];q=r.display;s=r.top||n-120;$tl=o("<div class='tg-timeline-envelope' id='"+r.id+"'><div class='titleBar'><div class='timeline-title'>"+r.title+"<div class='tg-timeline-env-buttons'><span class='timeline-info'>info</span><span class='expand-collapse'>expand/collapse</span></div></div></div></div>").appendTo(this._views.TICKS);
$tl.draggable({axis:"y",handle:".titleBar",stop:function(){Q.setTimelineProp(r.id,"top",o(this).css("top"))}}).css("top",s);s=$tl.height();o(".tg-timeline-envelope#"+r.id+" .titleBar .expand-collapse").click(function(){Q.expandCollapseTimeline(r.id)});L=$tl.children(".titleBar");m=aa+(r.bounds.first-fa)/Z;y=aa+(r.bounds.last-fa)/Z;L.css({top:s,left:m,width:y-m});if(q=="expanded")F=new timeglider.TGOrg;for(y=0;y<v.length;y++){m=this.getTimelineEventsByTick({tick:v[y],timeline:r});o.merge(qa,m)}for(y=
0;y<r.spans.length;y++){m=r.spans[y];m.start<ra&&m.end>ra&&o.inArray(m.id,qa)==-1&&qa.unshift(m.id)}for(m=0;m<qa.length;m++){y=p.eventPool["ev_"+qa[m]];if(this.passesFilters(y,va)==true){X=aa+(y.startdateObj.sec-fa)/Z;L=this.scaleToImportance(y.importance,va);if(q=="expanded"){y.width=y.titleWidth*L+18;if(y.span==true){y.spanwidth=(y.enddateObj.sec-y.startdateObj.sec)/Z;if(y.spanwidth>y.width)y.width=y.spanwidth}else y.spanwidth=0;y.fontsize=this.basicFontSize*L;y.left=X;L=0;if(y.image&&y.image_class==
"layout"){L=y.image_size.height+2;y.width=y.image_size.width+2}y.height=Math.ceil(y.fontsize)+L;y.top=s-y.height;F.addBlock(y,"sweep")}else if(q=="collapsed")S+="<div id='ev_"+y.id+"' class='timeglider-event-collapsed' style='top:"+(s-2)+"px;left:"+X+"px'></div>"}}if(q=="expanded"){S=F.getHTML("sweep");r.borg=F.getBorg()}S!="undefined"&&$tl.append(S);o(".timeglider-event-image-bar").each(function(){o(this).position({my:"top",at:"bottom",of:$tl,offset:"0, 0"}).css("left",0)})}Q.registerTitles()},appendTimelines:function(m){for(var r=
p._activeTimelines,s=this.dimensions.container.centerx,y,D,v,F,L,Q,S,X,aa=p._startSec,ra=p.getZoomInfo().spp,fa=p.getZoomInfo().level,Z=0;Z<r.length;Z++){y=p.timelinePool[r[Z]];Q=y.borg;$tl=o(".tg-timeline-envelope#"+y.id);L=$tl.height();idArr=this.getTimelineEventsByTick({tick:m,timeline:y});X=idArr.length;F=y.display;S="";for(i=0;i<X;i++){D=p.eventPool["ev_"+idArr[i]];v=s+(D.startdateObj.sec-aa)/ra;if(this.passesFilters(D,fa)==true)if(F=="expanded"){D.left=v;v=this.scaleToImportance(D.importance,
fa);D.fontsize=this.basicFontSize*v;D.width=D.titleWidth*v+18;if(D.span==true){D.spanwidth=(D.enddateObj.sec-D.startdateObj.sec)/ra;if(D.spanwidth>D.width)D.width=D.spanwidth}else D.spanwidth=0;D.top=L-timeglider.levelHeight;D.height=18;Q.addBlock(D,m.serial)}else if(F=="collapsed")S+="<div id='ev_"+D.id+"' class='timeglider-event-collapsed' style='top:"+(L-2)+"px;left:"+v+"px'></div>"}if(F=="expanded"){y.borg=Q.getBorg();S=Q.getHTML(m.serial)}$tl.append(S)}},expandCollapseTimeline:function(m){m=
p.timelinePool[m];m.display=m.display=="expanded"?"collapsed":"expanded";p.refresh()},eventModal:function(m){o("#"+m+"_modal").remove();var r=o("#"+m),s=p.eventPool[m],y="<div class='timeglider-ev-modal ui-widget-content shadow' id='"+m+"_modal'><div class='close-button'><img src='img/close.png'></div><div class='startdate'>"+s.startdate+"</div><h4 id='title'>"+s.title+"</h4><p>"+(s.image?"<img src='"+s.image+"'>":"")+s.description+"</p><ul class='timeglider-ev-modal-links'><li><a target='_blank' href='"+
s.link+"'>link</a></li></ul></div>";if(s.video)y="<div class='timeglider-ev-video-modal ui-widget-content shadow' id='"+m+"_modal'><div class='close-button'><img src='img/close.png'></div><iframe width = '100%' height='300' src='"+s.video+"'></iframe></div>";o(y).appendTo(this._views.TICKS).css("z-index",this.ztop++).position({my:"left bottom",at:"left top",of:r,offset:"0, -12",collision:"flip fit"})},parseHTMLTable:function(m){var r={},s=+new Date,y=[];o("#"+m).find("tr").each(function(D){var v=
o(this).children(),F;if(D===0)y=v.map(function(){return o(this).attr("class").replace(/^.*?\btg-(\S+)\b.*?$/,"$1")}).get();else{F={};v.each(function(L){F[y[L]]=o(this).text()});r["prefix"+s++]=F}});return r}};g.zoomTree=[{},{unit:"da",width:35E3,level:1,label:"5 hours"},{unit:"da",width:17600,level:2,label:"7 hours"},{unit:"da",width:8800,level:3,label:"10 hours"},{unit:"da",width:4400,level:4,label:"12 hours"},{unit:"da",width:2200,level:5,label:"14 hours"},{unit:"da",width:1100,level:6,label:"17 hours"},
...
width:76,level:95,label:"1 billion years"},{unit:"bill",width:603,level:96,label:"15 million years"},{unit:"bill",width:358,level:97,label:"30 million years"},{unit:"bill",width:213,level:98,label:"50 million years"},{unit:"bill",width:126,level:99,label:"80 million years"},{unit:"bill",width:76,level:100,label:"100 billion years"}];g.calculateSecPerPx=function(m){for(var r=1;r<m.length;r++){var s=m[r],y=0;switch(s.unit){case "da":y=86400;break;case "mo":y=2419200;break;case "ye":y=31536E3;break;
...
1);this.refresh()}o.publish("mediator.activeTimelinesChange")}};g.getLowHigh=function(C){var m,r,s=parseFloat(C[0]),y=s;for(m=0;m<C.length;m++){r=parseFloat(C[m]);if(r<y)y=r;if(r>s)s=r}return{high:s,low:y}};g.getStringWidth=function(C){var m=o("#timeglider-measure-span").html("");m.html(C+"");C=m.width()+4;m.html("");return C};g.getImageSize=function(C){var m=o("#timeglider-measure-span").html("");m.append("<img id='test_img' src='"+C+"'>");C=o("#test_img").width();var r=o("#test_img").height();m.html("");
