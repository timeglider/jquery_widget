...
*/
(function(){var r=this,F=r._,A={},o=Array.prototype,H=Object.prototype,B=o.slice,x=o.unshift,w=H.toString,h=H.hasOwnProperty,I=o.forEach,C=o.map,e=o.reduce,q=o.reduceRight,s=o.filter,z=o.every,g=o.some,k=o.indexOf,p=o.lastIndexOf;H=Array.isArray;var D=Object.keys,c=Function.prototype.bind,a=function(d){return new u(d)};typeof module!=="undefined"&&module.exports?(module.exports=a,a._=a):r._=a;a.VERSION="1.1.7";var n=a.each=a.forEach=function(d,i,m){if(d!=null)if(I&&d.forEach===I)d.forEach(i,m);else if(d.length===
...
return m};a.reduceRight=a.foldr=function(d,i,m,y){d==null&&(d=[]);if(q&&d.reduceRight===q)return y&&(i=a.bind(i,y)),m!==void 0?d.reduceRight(i,m):d.reduceRight(i);d=(a.isArray(d)?d.slice():a.toArray(d)).reverse();return a.reduce(d,i,m,y)};a.find=a.detect=function(d,i,m){var y;f(d,function(E,J,L){if(i.call(m,E,J,L))return y=E,true});return y};a.filter=a.select=function(d,i,m){var y=[];if(d==null)return y;if(s&&d.filter===s)return d.filter(i,m);n(d,function(E,J,L){i.call(m,E,J,L)&&(y[y.length]=E)});
return y};a.reject=function(d,i,m){var y=[];if(d==null)return y;n(d,function(E,J,L){i.call(m,E,J,L)||(y[y.length]=E)});return y};a.every=a.all=function(d,i,m){var y=true;if(d==null)return y;if(z&&d.every===z)return d.every(i,m);n(d,function(E,J,L){if(!(y=y&&i.call(m,E,J,L)))return A});return y};var f=a.some=a.any=function(d,i,m){i=i||a.identity;var y=false;if(d==null)return y;if(g&&d.some===g)return d.some(i,m);n(d,function(E,J,L){if(y|=i.call(m,E,J,L))return A});return!!y};a.include=a.contains=function(d,
...
i){if(d==null)return-1;if(p&&d.lastIndexOf===p)return d.lastIndexOf(i);for(var m=d.length;m--;)if(d[m]===i)return m;return-1};a.range=function(d,i,m){arguments.length<=1&&(i=d||0,d=0);m=arguments[2]||1;for(var y=Math.max(Math.ceil((i-d)/m),0),E=0,J=Array(y);E<y;){J[E++]=d;d+=m}return J};a.bind=function(d,i){if(d.bind===c&&c)return c.apply(d,B.call(arguments,1));var m=B.call(arguments,2);return function(){return d.apply(i,m.concat(B.call(arguments)))}};a.bindAll=function(d){var i=B.call(arguments,
...
*/
(function(){var r=this,F=r.Backbone,A;A=typeof exports!=="undefined"?exports:r.Backbone={};A.VERSION="0.5.3";var o=r._;if(!o&&typeof require!=="undefined")o=require("underscore")._;var H=r.jQuery||r.Zepto;A.noConflict=function(){r.Backbone=F;return this};A.emulateHTTP=false;A.emulateJSON=false;A.Events={bind:function(c,a,n){var f=this._callbacks||(this._callbacks={});(f[c]||(f[c]=[])).push([a,n]);return this},unbind:function(c,a){var n;if(c){if(n=this._callbacks)if(a){n=n[c];if(!n)return this;for(var f=
...
f;f.__super__=c.prototype;return f},k=function(c){if(!c||!c.url)return null;return o.isFunction(c.url)?c.url():c.url},p=function(){throw Error('A "url" property or function must be specified');},D=function(c,a,n){return function(f){c?c(a,f,n):a.trigger("error",a,f,n)}}}).call(this);/*
...
if(G){v=c;l=r.template[l]||r.template(null,l);p={}}else if(!l){l=v.tmpl;k[v.key]=v;v.nodes=[];v.wrapped&&B(v,v.wrapped);return r(A(v,null,v.tmpl(r,v)))}if(!l)return[];if(typeof j==="function")j=j.call(v||{});u&&u.wrapped&&B(u,u.wrapped);j=r.isArray(j)?r.map(j,function(d){return d?F(u,v,l,d):null}):[F(u,v,l,j)];return G?r(A(v,null,j)):j},tmplItem:function(l){var j;if(l instanceof r)l=l[0];for(;l&&l.nodeType===1&&!(j=r.data(l,"tmplItem"))&&(l=l.parentNode););return j||c},template:function(l,j){if(j){if(typeof j===
...
function iPadTouchStart(r){var F=r.changedTouches[0],A="mouseover",o=document.createEvent("MouseEvent");o.initMouseEvent(A,true,true,window,1,F.screenX,F.screenY,F.clientX,F.clientY,false,false,false,false,0,null);F.target.dispatchEvent(o);A="mousedown";o=document.createEvent("MouseEvent");o.initMouseEvent(A,true,true,window,1,F.screenX,F.screenY,F.clientX,F.clientY,false,false,false,false,0,null);F.target.dispatchEvent(o);if(tapValid){window.clearTimeout(tapTimeout);if(F.target==lastTap){lastTap=
null;tapValid=false;A="click";o=document.createEvent("MouseEvent");o.initMouseEvent(A,true,true,window,1,F.screenX,F.screenY,F.clientX,F.clientY,false,false,false,false,0,null);F.target.dispatchEvent(o);A="dblclick";o=document.createEvent("MouseEvent");o.initMouseEvent(A,true,true,window,1,F.screenX,F.screenY,F.clientX,F.clientY,false,false,false,false,0,null);F.target.dispatchEvent(o)}else{lastTap=F.target;tapValid=true;tapTimeout=window.setTimeout("cancelTap();",600);startHold(r)}}else{lastTap=
...
Q)}else N=o(M[0],Q);Q=N;if(Q<0||Q>6)return null;break;case "zzz":M=P.split(/:/);if(M.length!==2)return null;E=parseInt(M[0],10);if(E<-12||E>13)return null;M=parseInt(M[1],10);if(M<0||M>59)return null;E=E*60+(P.indexOf("-")===0?-M:M);break;case "z":case "zz":E=N;if(E<-12||E>13)return null;E*=60;break;case "g":case "gg":if(!P||!j.eras)return null;P=r(P.toLowerCase());M=0;for(N=j.eras.length;M<N;M++)if(P===j.eras[M].name.toLowerCase()){f=M;break}if(f===null)return null}}}m=new Date;y=(J=j.convert)?J.fromGregorian(m)[0]:
m.getFullYear();if(O===null)O=y;else if(j.eras)O+=j.eras[f||0].offset;if(K===null)K=0;if(l===null)l=1;if(J){m=J.toGregorian(O,K,l);if(m===null)return null}else{m.setFullYear(O,K,l);if(m.getDate()!==l)return null;if(Q!==null&&m.getDay()!==Q)return null}if(i&&u<12)u+=12;m.setHours(u,v,G,d);if(E!==null){j=m.getMinutes()-(E+m.getTimezoneOffset());m.setHours(m.getHours()+parseInt(j/60,10),j%60)}return m}function s(f,l,j){function u(M,N){var S;S=M+"";if(N>1&&S.length<N){S=J[N-2]+S;return S.substr(S.length-
...
Q=0,P=e(),R;if(!y&&i)R=i.fromGregorian(f);for(;;){E=P.lastIndex;i=P.exec(l);E=l.slice(E,i?i.index:l.length);Q+=I(E,j);if(!i)break;if(Q%2)j.push(i[0]);else{E=i[0];i=E.length;switch(E){case "ddd":case "dddd":j.push((i===3?d.days.namesAbbr:d.days.names)[f.getDay()]);break;case "d":case "dd":L=true;j.push(u(G(f,2),i));break;case "MMM":case "MMMM":E=G(f,1);j.push(d.monthsGenitive&&v()?d.monthsGenitive[i===3?"namesAbbr":"names"][E]:d.months[i===3?"namesAbbr":"names"][E]);break;case "M":case "MM":j.push(u(G(f,
1)+1,i));break;case "y":case "yy":case "yyyy":E=R?R[0]:h(f,d,B(f,m),y);if(i<4)E%=100;j.push(u(E,i));break;case "h":case "hh":E=f.getHours()%12;if(E===0)E=12;j.push(u(E,i));break;case "H":case "HH":j.push(u(f.getHours(),i));break;case "m":case "mm":j.push(u(f.getMinutes(),i));break;case "s":case "ss":j.push(u(f.getSeconds(),i));break;case "t":case "tt":E=f.getHours()<12?d.AM?d.AM[0]:" ":d.PM?d.PM[0]:" ";j.push(i===1?E.charAt(0):E);break;case "f":case "ff":case "fff":j.push(u(f.getMilliseconds(),3).substr(0,
i));break;case "z":case "zz":E=f.getTimezoneOffset()/60;j.push((E<=0?"+":"-")+u(Math.floor(Math.abs(E)),i));break;case "zzz":E=f.getTimezoneOffset()/60;j.push((E<=0?"+":"-")+u(Math.floor(Math.abs(E)),2)+":"+u(Math.abs(f.getTimezoneOffset()%60),2));break;case "g":case "gg":d.eras&&j.push(d.eras[B(f,m)].name);break;case "/":j.push(d["/"]);break;default:throw"Invalid date format pattern '"+E+"'.";}}}return j.join("")}var z={},g={en:{}};g["default"]=g.en;z.extend=function(f){for(var l=arguments[1]||{},
j=2,u=arguments.length;j<u;j++){var v=arguments[j];if(v)for(var G in v){var d=v[G];if(typeof d!=="undefined")if(f&&(n.call(d)==="[object Object]"||A(d))){var i=l[G];i=i&&(n.call(i)==="[object Object]"||A(i))?i:A(d)?[]:{};l[G]=this.extend(true,i,d)}else l[G]=d}}return l};z.findClosestCulture=function(f){var l;if(!f)return this.culture||this.cultures["default"];if(n.call(f)==="[object String]")f=f.split(",");if(A(f)){var j,u=this.cultures,v=f,G,d=v.length,i=[];for(G=0;G<d;G++){f=r(v[G]);f=f.split(";");
j=r(f[0]);if(f.length===1)f=1;else{f=r(f[1]);if(f.indexOf("q=")===0){f=f.substr(2);f=parseFloat(f,10);f=isNaN(f)?0:f}else f=1}i.push({lang:j,pri:f})}i.sort(function(y,E){return y.pri<E.pri?1:-1});for(G=0;G<d;G++){j=i[G].lang;if(l=u[j])return l}for(G=0;G<d;G++){j=i[G].lang;do{v=j.lastIndexOf("-");if(v===-1)break;j=j.substr(0,v);if(l=u[j])return l}while(1)}for(G=0;G<d;G++){j=i[G].lang;for(var m in u){v=u[m];if(v.language==j)return v}}}else if(typeof f==="object")return f;return l||null};z.preferCulture=
...
f;if(!l||l==="i")f=j.name.length?u.toLocaleString():u.toString();else{l=l||"D";j=j.numberFormat;f=Math.abs(u);var v=-1;if(l.length>1)v=parseInt(l.slice(1),10);var G=l.charAt(0).toUpperCase(),d;switch(G){case "D":l="n";if(v!==-1)f=F(""+f,v,true);if(u<0)f=-f;break;case "N":d=j;case "C":d=d||j.currency;case "P":d=d||j.percent;l=u<0?d.pattern[0]:d.pattern[1]||"n";if(v===-1)v=d.decimals;b:{var i=f*(G==="P"?100:1);G=d;d=G.groupSizes;f=d[0];u=1;var m=Math.pow(10,v),y=Math.round(i*m)/m;isFinite(y)||(y=i);
m="";m=(y+"").split(/e/i);y=m.length>1?parseInt(m[1],10):0;i=m[0];m=i.split(".");i=m[0];m=m.length>1?m[1]:"";if(y>0){m=F(m,y,false);i+=m.slice(0,y);m=m.substr(y)}else if(y<0){y=-y;i=F(i,y+1);m=i.slice(-y,i.length)+m;i=i.slice(0,-y)}m=v>0?G["."]+(m.length>v?m.slice(0,v):F(m,v)):"";v=i.length-1;G=G[","];for(y="";v>=0;){if(f===0||f>v){f=i.slice(0,v+1)+(y.length?G+y+m:m);break b}y=i.slice(v-f+1,v+1)+(y.length?G+y:"");v-=f;if(u<d.length){f=d[u];u++}}f=i.slice(0,v+1)+G+y+m}break;default:throw"Bad number format specifier: "+
...
-1){f=f.replace(j.numberFormat.currency.symbol,"");f=f.replace(j.numberFormat.currency["."],j.numberFormat["."])}f=r(f);if(D.test(f))u=parseFloat(f,l);else if(!l&&c.test(f))u=parseInt(f,16);else{j=H(f,v,v.pattern[0]);l=j[0];j=j[1];if(l===""&&v.pattern[0]!=="-n"){j=H(f,v,"-n");l=j[0];j=j[1]}l=l||"+";var G;f=j.indexOf("e");if(f<0)f=j.indexOf("E");if(f<0){G=j;f=null}else{G=j.substr(0,f);f=j.substr(f+1)}var d=v["."],i=G.indexOf(d);if(i<0){j=G;G=null}else{j=G.substr(0,i);G=G.substr(i+d.length)}d=v[","];
j=j.split(d).join("");i=d.replace(/\u00A0/g," ");if(d!==i)j=j.split(i).join("");l+=j;if(G!==null)l+="."+G;if(f!==null){v=H(f,v,"-n");l+="e"+(v[0]||"+")+v[1]}if(a.test(l))u=parseFloat(l)}return u};z.parseDate=function(f,l,j){j=this.findClosestCulture(j);var u,v;if(l){if(typeof l==="string")l=[l];if(l.length){v=0;for(var G=l.length;v<G;v++){var d=l[v];if(d)if(u=q(f,d,j))break}}}else{l=j.calendar.patterns;for(v in l)if(u=q(f,l[v],j))break}return u||null};var k=z.cultures=z.cultures||{};k=k["default"]=
...
151;break;case 7:n=181;break;case 8:n=212;break;case 9:n=243;break;case 10:n=273;break;case 11:n=304;break;case 12:n=334}if(c>2)if(C.isLeapYear(a))n+=1;return n}var z=e.ye,g=e.mo;e=e.da;var k=0;if(H[z+"-"+g+"-"+e])return H[z+"-"+g+"-"+e];if(z>=0){if(z==0)z=1;k=Math.floor(z/400)*146097;var p=parseInt(q(z-z%400,z)),D=parseInt(s(g,z));k=k+p+D+e-366}else if(z<0)k=C.getBCRataDie({ye:z,mo:g,da:e});return H[z+"-"+g+"-"+e]=k};C.monthNamesLet=["","J","F","M","A","M","J","J","A","S","O","N","D"];C.monthsDayNums=
...
16)e+=":00"+q;return e}};C.parse8601=function(e){var q,s,z,g,k;k=function(D){return D?parseInt(D.replace("-",""),10):0};var p=e.match(I);e=parseInt(p[1]);if(!e)return{error:"invalid date; no year provided"};q=k(p[2])||1;s=k(p[3])||1;z=k(p[4])||1;g=k(p[5])||0;k=k(p[6])||0;tz_pm=p[7]||"+";tz_ho=parseInt(p[8],10)||0;if(tz_pm=="-")tz_ho*=-1;tz_mi=parseInt(p[9],10)||0;if(tz_pm=="-")tz_mi*=-1;return{ye:e,mo:q,da:s,ho:z,mi:g,se:k,tz_ho:tz_ho,tz_mi:tz_mi}};C.getLastDayOfMonth=function(e,q){var s=[0,31,28,
...
function(){p.intervalMachine("pan",{type:"clear",fn:p.pan,callback:"resetTicksHandle"})})},intervalMachine:function(g,k){var p=this;if(k.type==="clear"){clearInterval(I[g]);if(k.callback)p[k.callback]()}else I[g]=setInterval(function(){k.fn.apply(p,k.args)},k.intvl)},invSliderVal:function(g){return Math.abs(g-101)},pan:function(g){g=g||20;$t=h(q);newPos=$t.position().left+g;$t.css({left:newPos});o.setTicksOffset(newPos)},registerTitles:function(){var g,k,p,D,c,a,n,f,l,j=h(e).offset().left;h(".timeglider-event-spanning").each(function(){g=
h(this).offset().left-j;k=h(this).outerWidth();a=h(".timeglider-event-title",this);p=a.outerWidth()+5;D=a.siblings(".timeglider-event-spanner").outerWidth();D>p&&g<0&&Math.abs(g)<k-p&&a.css({marginLeft:-1*g+5})});h.browser.msie&&parseInt(h.browser.version);h(".tg-timeline-envelope").each(function(){n=h(this);f=n.offset().left-j;$tb=n.find(".titleBar");c=$tb.data("lef");l=-1*(c+f);$ti=$tb.find(".timeline-title");l>0&&l<$tb.width()-$ti.width()&&$ti.css({marginLeft:l+5})})},registerDragging:function(){var g=
o.startSec,k=h(q).position().left,p=o.getZoomInfo().spp;g=new F(g-k*p);o.setFocusDate(g);this.displayFocusDate()},setupFilter:function(){var g=this;$filter=h.tmpl(g._templates.filter_modal,{}).appendTo(g._views.CONTAINER);$filter.css("z-index",g.ztop++).position({my:"right bottom",at:"right top",of:h(g._views.FILTER_BT),offset:"-8, -12"}).hide();h(e).delegate(".timeglider-filter-box .close-button","click",function(){$filter.fadeOut()});h(g._views.FILTER_BT).bind("click",function(){$filter.fadeIn();
...
this.render,this)},tagName:"li",className:"timeglider-timeline-list-item",template:"${title}",events:{click:"toggleTimeline"},toggleTimeline:function(){o.toggleTimeline(this.model.get("id"))},render:function(){var D=this.model.get("id");h(this.el).html(h.tmpl(this.template,this.model.attributes)).data("timeline_id",D);return this}});h(g._views.TIMELINE_MENU_UL).html("");_.each(o.timelineCollection.models,function(D){h(g._views.TIMELINE_MENU_UL).append((new p({model:D})).render().el)});k.position({my:"right bottom",
at:"right top",of:h(g._views.TIMELINE_LIST_BT),offset:"-8, -12"}).hide();h(e).delegate(".timeglider-timeline-menu .close-button","click",function(){k.fadeOut()}).delegate(this._views.TIMELINE_LIST_BT,"click",function(){k.fadeIn()})},getTimezonePulldown:function(g,k){var p="<select name='timezone' id='"+g+"'>",D=false,c="selected";h.map(F.timezones,function(a){if(k==a.offset&&D==false){c="selected";D=true}else c="";p+="<option value='"+a.offset+"' "+c+">"+a.name+"</option>"});p+="</select>";return p},
...
true&&h("<div>").appendTo(this._views.SLIDER_CONTAINER).addClass("timeglider-zoomlevel-display").html("&nbsp;");var k=this;g=k.invSliderVal(g);var p=o.max_zoom,D=o.min_zoom;h(this._views.SLIDER).css("height",(1+p-D)*3).slider({steps:100,handle:h(".knob"),animate:300,orientation:"vertical",min:k.invSliderVal(p),max:k.invSliderVal(D),value:g,start:function(){k.sliderActive=true},stop:function(){k.sliderActive=false},change:function(){},slide:function(c,a){o.setZoomLevel(k.invSliderVal(a.value))}})}},
...
focus_date:g});for(g=1;g<=k;g+=1){this.addTick({type:p});p=p=="l"?"r":"l"}o.setTicksReady(true);this.displayFocusDate()},addTick:function(g){var k=0,p=0,D=p=0,c=0,a=0,n=a="",f={};a={};D={};var l={};a={};f="";l=o.getZoomInfo().unit;var j=o.getZoomInfo().width;c=o.getFocusDate();D=parseInt(this.dimensions.tick.top);c=o.addToTicksArray({type:g.type,unit:l},c);if(l=="mo"){a=F.getMonthAdj(c,j);j=a.width;k=a.days}this.tickNum++;if(g.type=="init"){a=this.tickOffsetFromDate(o.getZoomInfo(),o.getFocusDate(),
j);this.leftside=p=Math.ceil(this.dimensions.container.centerx+a);this.rightside=p+j}else if(g.type=="l")this.leftside=p=Math.floor(this.leftside-j);else if(g.type=="r"){p=Math.floor(this.rightside);this.rightside+=j}o.getTickBySerial(c).width=j;o.getTickBySerial(c).left=p;a=this._views.PLACE+"_"+l+"_"+c+"-"+this.tickNum;a=h("<div class='timeglider-tick' id='"+a+"'><div class='timeglider-tick-label' id='label'></div></div>").appendTo(q);a.css({width:j,left:p,top:D});D=this.getTickMarksInfo({unit:l,
width:j});D=k||D.tperu;p=j/D;if(p>5)for(g=k=0;g<D;g++){n="&nbsp;";if(p>16)if(l=="da")n=this.getHourLabelFromHour(g,p);else if(l=="mo")n=g+1;else if(l=="ye")n=p>30?"&nbsp;"+F.monthNamesAbbr[g+1]:"&nbsp;"+F.monthNamesLet[g+1];f+="<div class='timeglider-tick-sub-label' style='left:"+k+"px;width:"+p+"px'>"+n+"</div>";k+=p}f&&a.append("<div style='background-color:none;height:24px;width:"+(j+10)+"px;position:absolute;top:15px;left:0;overflow:hidden'>"+f+"</div>");l={unit:l,width:j,serial:c};f=this.getDateLabelForTick(l);
a.children("#label").text(f);return l},setCenterline:function(){o.options.show_centerline===true?h(this._views.CENTERLINE).css({height:this.dimensions.container.height,left:this.dimensions.container.centerx}):h(this._views.CENTERLINE).css({display:"none"})},getHourLabelFromHour:function(g,k){var p="",D=g,c="";if(k<16)return"";else{if(g>12)D=g-12;else if(g==0)D=12;if(k>30)p=g>11?" pm":" am";if(k>200){sl4hd=k/4-4;return"<div class='minutes' style='width:"+sl4hd+"px'>"+D+":00 "+p+"</div><div class='minutes' style='width:"+
sl4hd+"px'>"+D+":15 "+p+"</div><div class='minutes' style='width:"+sl4hd+"px'>"+D+":30 "+p+"</div><div class='minutes' style='width:"+sl4hd+"px'>"+D+":45 "+p+"</div>"}else return D+(k>60?":00":"")+p}},getTickMarksInfo:function(g){switch(g.unit){case "da":g=24;break;case "mo":g=30;break;case "ye":g=12;break;default:g=10}return{tperu:g}},getDateLabelForTick:function(g){var k=g.serial,p=g.width;switch(g.unit){case "bill":return k==0?"1":k>0?k+" billion":k+" b.y. bce";case "hundredmill":return k==0?"1":
k>0?k+"00 million":k+"00 m.y. bce";case "tenmill":return k==0?"1":k>0?k+"0 million":k+"0 m.y. bce";case "mill":return k==0?"1":k>0?k+" million":k+" m.y. bce";case "hundredthou":return k==0?"1":k>0?k+"00,000":k+"00,000 bce";case "tenthou":return k==0?"1":k>0?k+"0,000":k+"0,000 bce";case "thou":return k==0?"1("+k+")":k>0?k+"000":k+"000 bce";case "ce":return k==0?"1("+k+")":k>0?k+"00":k+"00 bce";case "de":return k>120?k*10+"s":k*10;case "ye":return k;case "mo":g=F.getDateFromMonthNum(k);return p<120?
F.monthNamesAbbr[g.mo]+" "+g.ye:F.monthNames[g.mo]+", "+g.ye;case "da":g=new F(F.getDateFromRD(k));return p<120?F.monthNamesAbbr[g.mo]+" "+g.da+", "+g.ye:F.monthNames[g.mo]+" "+g.da+", "+g.ye;default:return g.unit+":"+k+":"+p}},tickHangies:function(){var g=h(q).position().left,k=this.rightside+g-this.dimensions.container.width;if(this.leftside+g>-100){g=this.addTick({type:"l"});this.appendTimelines(g)}else if(k<100){g=this.addTick({type:"r"});this.appendTimelines(g)}},tickOffsetFromDate:function(g,
k,p){switch(g.unit){case "da":k=k.ho/24+k.mi/1440;p*=k;break;case "mo":g=F.getMonthDays(k.mo,k.ye);k=(k.da-1)/g+k.ho/(24*g)+k.mi/(1440*g);p*=k;break;case "ye":k=((k.mo-1)*30+k.da)/365;p*=k;break;case "de":k=k.ye%10/10+k.mo/120;p*=k;break;case "ce":k=k.ye%100/100+k.mo/1200;p*=k;break;case "thou":k=k.ye%1E3/1E3;p*=k;break;case "tenthou":k=k.ye%1E4/1E4;p*=k;break;case "hundredthou":k=k.ye%1E5/1E5;p*=k;break;default:p=0}return-1*p},resetTicksHandle:function(){h(this._views.HANDLE).offset({left:h(e).offset().left})},
easeOutTicks:function(){Math.abs(B)>5&&h(q).animate({left:"+="+3*B},1E3,"easeOutQuad",function(){debug.trace("stopping easing","note")})},getTimelineEventsByTick:function(g){var k=g.tick.unit,p=g.tick.serial;g=o.eventCollection.getTimelineHash(g.timeline.timeline_id);return g[k][p]&&g[k][p].length>0?g[k][p]:0},passesFilters:function(g,k){var p=true,D="",c=[],a;D="";c=[];if(k<g.low_threshold||k>g.high_threshold)return false;if(D=o.filters.include){c=D.split(",");p=false;for(a=0;a<c.length;a++){D=RegExp(h.trim(c[a]),
"i");if(g.title.match(D))p=true}}if(D=o.filters.exclude){c=D.split(",");for(a=0;a<c.length;a++){D=RegExp(h.trim(c[a]),"i");if(g.title.match(D))p=false}}D=g.icon;if(o.filters.legend.length>0)if(h.inArray(D,o.filters.legend)==-1)p=false;return p},freshTimelines:function(){var g=this,k,p,D,c=o.activeTimelines,a=o.ticksArray,n="",f;p="";var l=g.dimensions.container.centerx;k=g.dimensions.container.width;var j=o.getFocusDate().sec,u=o.getZoomInfo().spp;o.getZoomInfo();p=[];var v=[],G=j-Math.floor(u*(k/
2));D=f=0;for(var d,i=0,m=g.dimensions.container.height,y=i=0;y<c.length;y++){v=[];p=o.timelineCollection.get(c[y]);k=p.attributes;d=k.display;i=k.top?parseInt(k.top.replace("px","")):m-g.initTimelineVOffset;p=new r.TG_TimelineView({model:p});f=o.timeOffset.seconds/u;$tl=h(p.render().el).appendTo(q);$tl.draggable({axis:"y",handle:".titleBar",stop:function(){var E=h(this).css("top"),J=h(this).attr("id");o.timelineCollection.get(J).set({top:E});o.refresh()}}).css({top:i,left:f});f=$tl.find(".titleBar");
if(typeof k.bounds!="undefined"){p=l+(k.bounds.first-j)/u;D=l+(k.bounds.last-j)/u}else{p=l;D=l+300}D-=p;f.css({top:void 0,left:p,width:D}).data({lef:p,wid:D});if(d=="expanded")k.borg=n=new timeglider.TG_Org;for(f=0;f<a.length;f++){p=this.getTimelineEventsByTick({tick:a[f],timeline:k});h.merge(v,p)}_.each(k.spans,function(E){E.start<G&&E.end>G&&h.inArray(E.id,v)==-1&&v.unshift(E.id)});p=this.compileTickEventsAsHtml(k,v,0,"sweep");i=H.event_overflow=="scroll"?0:k.hasImagesAbove?i-g.imageLaneHeight:
i;if(d=="expanded"){p=n.getHTML("sweep",i);k.borg=n.getBorg()}p!="undefined"&&$tl.append(p.html);setTimeout(function(){g.registerEventImages($tl)},3)}g.registerTitles();h.publish("viewer.rendered")},appendTimelines:function(g){for(var k=o.activeTimelines,p,D="",c=0;c<k.length;c++){p=o.timelineCollection.get(k[c]).attributes;idArr=this.getTimelineEventsByTick({tick:g,timeline:p});D=this.compileTickEventsAsHtml(p,idArr,g.serial,"append");if(p.display=="expanded")D=p.borg.getHTML(g.serial,p.top);p=h(e+
" .tg-timeline-envelope#"+p.id).append(D.html);this.registerEventImages(p)}h.publish("viewer.rendered")},compileTickEventsAsHtml:function(g,k,p,D){var c=0,a=this.dimensions.container.centerx,n=g.display,f=$tl.height();stuff="";foSec=o.startSec;spp=o.getZoomInfo().spp;zl=o.getZoomInfo().level;buffer=20;img_ht=0;borg=g.borg;block_arg="sweep";if(D=="append")block_arg=p;for(p=0;p<k.length;p++){ev=o.eventCollection.get(k[p]).attributes;if(this.passesFilters(ev,zl)===true){c=a+(ev.startdateObj.sec-foSec)/
...
createEventLinksMenu:function(g){if(!g)return"";var k="",p=0,D="",c="";if(typeof g=="string")k="<li><a href='"+g+"' target='_blank'>link</a></li>";else if(typeof g=="object")for(p=0;p<g.length;p++){D=g[p].url;c=g[p].label;k+="<li><a href='"+D+"' target='_blank'>"+c+"</a></li>"}return k},eventModal:function(g){h(e+" #"+g+"_modal").remove();var k=this,p=false,D=false,c="";c={};c=0;c=h("#"+g);var a=o.eventCollection.get(g).attributes,n=a.modal_type||H.event_modal.type,f=a.image&&a.image.src?"<img src='"+
a.image.src+"'>":"",l=this.createEventLinksMenu(a.link);g={title:a.title,description:a.description,id:g,dateline:k.getEventDateLine(a),links:l,image:f};if(a.video){g.video=a.video;n="full";D=true;g.video=a.video}else if(a.map&&a.map.latlong){p=true;n="full"}switch(n){case "full":g=h.tmpl(k._templates.event_modal_full,g);g.appendTo(e).css({"z-index":k.ztop++}).position({my:"left top",at:"left top",of:e,offset:"0, 0",collision:"none none"});if(p==true){g.find("#insert").append("<div id='map_modal_map'></div>");
c=a.map.zoom||12;llarr=String(a.map.latlong).split(",");map_ll=new google.maps.LatLng(parseFloat(llarr[0]),parseFloat(llarr[1]));c={zoom:c,center:map_ll,mapTypeId:google.maps.MapTypeId.ROADMAP};c=new google.maps.Map(h("#map_modal_map")[0],c);if(a.map.markers)for(p=0;p<a.map.markers.length;p++){D=a.map.markers[p];new google.maps.MarkerImage(D.image,new google.maps.Size(24,32),new google.maps.Point(0,0),new google.maps.Point(0,32));g=D.latlong.split(",");g=new google.maps.LatLng(g[0],g[1]);D=new google.maps.Marker({position:g,
map:c,icon:D.icon,title:D.title,zIndex:D.zIndex})}}else D==true&&g.find("#insert").append("<iframe width='100%' height='300' src='"+a.video+"'></iframe></div>");a=k.dimensions.container.height;p=k.dimensions.container.width;D=h(e+" .full_modal_panel");g=D.width();c=D.height();D.css({top:(a-c)/2+"px",left:(p-g)/2+"px"});break;case "video":g=h.tmpl(k._templates.event_modal_video,g);g.appendTo(q).css("z-index",k.ztop++).position({my:"right center",at:"left center",of:c,offset:"-12, -1",collision:"flip fit"}).hover(function(){h(this).css("z-index",
k.ztop++)});break;default:g=h.tmpl(k._templates.event_modal_small,g);g.appendTo(q).css("z-index",k.ztop++).position({my:"center top",at:"center top",of:e,offset:"0, 100",collision:"fit fit"}).draggable().hover(function(){h(this).css("z-index",k.ztop++)})}},legendModal:function(g){var k=o.timelineCollection.get(g).attributes.legend,p=0,D="",c="",a="";p="";for(p=0;p<k.length;p++){D=H.icon_folder+k[p].icon;c=k[p].title;a+="<li><img class='legend-icon' src='"+D+"'><span class='legend-info'>"+c+"</span></li>"}g=
...
icon:n})})},parseHTMLTable:function(g){var k={},p=+new Date,D=[];h("#"+g).find("tr").each(function(c){var a=h(this).children(),n;if(c===0)D=a.map(function(){return h(this).attr("class").replace(/^.*?\btg-(\S+)\b.*?$/,"$1")}).get();else{n={};a.each(function(f){n[D[f]]=h(this).text()});k["prefix"+p++]=n}});return k}};r.TG_TimelineView=Backbone.View.extend({initialize:function(){var g=this;this.model.bind("change:title",function(){h(g.el).find(".timeline-title-span").text(g.model.get("title"))});this.model.bind("destroy",
