...
this.rightside=this.leftside=this.tickNum=0;this.timeoout_id=1;this.sliderActive=false;this.ztop=1E3;this.filterBoxActivated=false;this.buildSlider();this.setupFilter();this.setPanButton(j(".timeglider-pan-right"),-30);this.setPanButton(j(".timeglider-pan-left"),30);j(this._views.TRUCK).bind("dblclick",function(a){q.registerUIEvent({name:"dblclick",event:a})}).bind("mousewheel",function(a,k){var g=Math.ceil(-1*k*3),i=q.getZoomLevel();q.setZoomLevel(i+g);return false});j(t).draggable({axis:"x",start:function(){},
drag:function(){H=Math.floor(j(this).position().left);q.setTicksOffset(H);z=H-v;v=H;var a=p.dragScopeState;if(a[0]=="over-left")j(t).css("left",a[1]-5);else if(a[0]=="over-right")j(t).css("left",a[1]+5);else return true},stop:function(){p.resetTicksHandle();p.registerDragging()}}).delegate(n+" .timeglider-timeline-event",G,function(){var a=j(this).attr("id"),k=q.eventCollection.get(a).attributes;if(timeglider.mode!="authoring")if(k.click_callback){a=k.click_callback.split(".");var g=a.length;if(g==
1)window[a[0]](k);else if(g==2)window[a[0]][a[1]](k);else if(g==3)window[a[0]][a[1]][a[2]](k)}else p.eventModal(a)}).delegate(".timeglider-timeline-event","mouseover",function(){var a=q.eventCollection.get(j(this).attr("id")).attributes;p.eventHover(j(this),a)}).delegate(".timeglider-timeline-event","mouseout",function(){var a=q.eventCollection.get(j(this).attr("id")).attributes;p.eventUnHover(j(this),a)}).delegate(".timeglider-event-collapsed","hover",function(){q.eventCollection.get(j(this).attr("id"))});
p.setCenterline();j.subscribe(o+".mediator.ticksOffsetChange",function(){p.tickHangies();p.registerDragging();p.registerTitles()});j.subscribe(o+".mediator.focusToEvent",function(){});j.subscribe(o+".mediator.zoomLevelChange",function(){p.tickNum=0;p.leftside=0;var a=q.getZoomLevel();j(p._views.SLIDER).slider("value",p.invSliderVal(a));p.displayZoomLevel(a);p.castTicks("zoomLevelChange")});j.subscribe(o+".viewer.rendered",function(){});j.subscribe(o+".mediator.ticksReadySignal",function(){q.ticksReady===
true&&p.freshTimelines()});j.subscribe(o+".mediator.refreshSignal",function(){p.tickNum=0;p.leftside=0;p.castTicks("refreshSignal")});j.subscribe(o+".mediator.ticksArrayChange",function(){});j.subscribe(o+".mediator.scopeChange",function(){var a=q.getScope(),k=a.timelineBounds;a=a.focusDateSec;p.dragScopeState=a>k.last?["over-right",j(t).position().left]:a<k.first?["over-left",j(t).position().left]:["okay",0]});j.subscribe(o+".mediator.focusDateChange",function(){this.displayFocusDate()});j.subscribe(o+
".mediator.timelineDataLoaded",function(){j(".timeglider-loading").fadeOut(500);p.buildSettingsMenu();p.buildTimelineMenu()});j.subscribe(o+".mediator.activeTimelinesChange",function(){j(p._views.TIMELINE_MENU_UL+" li").each(function(){var a=j(this).data("timeline_id");j.inArray(a,q.activeTimelines)!=-1?j(this).addClass("activeTimeline"):j(this).removeClass("activeTimeline")})});j.subscribe(o+".mediator.filterChange",function(){});if(j.support.touch){j("#"+e).addTouch();var b=document.getElementById(e);
b.addEventListener("gesturestart",function(a){a.preventDefault();j("#output").append("<br>gesture zoom:"+q.getZoomLevel())},false);b.addEventListener("gestureend",function(a){a.preventDefault();j("#output").append("<br>gesture end:"+q.getZoomLevel())},false);b.addEventListener("gesturechange",function(a){a.preventDefault();a.preventDefault();if(q.gesturing===false){q.gesturing=true;q.gestureStartZoom=q.getZoomLevel()}q.setZoomLevel(Math.ceil(q.gestureStartZoom/a.scale))},false)}};s.TG_PlayerView.prototype=
{resize:function(){var d=j(A).height();j(n).height(d);this.dimensions=this.getWidgetDimensions();q.setDimensions(this.dimensions);this.setCenterline();q.refresh()},getWidgetDimensions:function(){var d=j(n),l=d.width(),p=Math.floor(l/2)+1,b=d.height(),a=Math.floor(b/2),k=this.tick_height,g=d.position().left;d=d.offset();var i=E.show_footer==true?j(this._views.FOOTER).height():0;return{container:{width:l,height:b,centerx:p,centery:a,left:g,offset:d},tick:{top:b-i-k},footer:{height:i}}},scaleToImportance:function(d,
l){return d/l},displayZoomLevel:function(){var d=q.getZoomLevel();d>0&&E.display_zoom_level==true&&j(this._views.ZOOM_DISPLAY).text(d)},displayFocusDate:function(){var d=q.getFocusDate();j(y).text(d.format("d MMM yyyy",false))},setPanButton:function(d,l){var p=this;j(d).live("mousedown",function(){p.intervalMachine("pan",{type:"set",fn:p.pan,args:[l],intvl:33})}).live("mouseup",function(){p.intervalMachine("pan",{type:"clear",fn:p.pan,callback:"resetTicksHandle"})}).live("mouseout",function(){p.intervalMachine("pan",
{type:"clear",fn:p.pan,callback:"resetTicksHandle"})})},intervalMachine:function(d,l){var p=this;if(l.type==="clear"){clearInterval(w[d]);if(l.callback)p[l.callback]()}else w[d]=setInterval(function(){l.fn.apply(p,l.args)},l.intvl)},invSliderVal:function(d){return Math.abs(d-101)},pan:function(d){var l=d||20;d=j(t);l=d.position().left+l;d.css({left:l});q.setTicksOffset(l)},registerTitles:function(){var d,l,p,b,a,k,g,i,h,r,u,B=j(n).offset().left;j(".timeglider-event-spanning").each(function(){d=j(this).offset().left-
B;l=j(this).outerWidth();k=j(".timeglider-event-title",this);p=k.outerWidth()+5;b=k.siblings(".timeglider-event-spanner").outerWidth();b>p&&d<0&&Math.abs(d)<l-p&&k.css({marginLeft:-1*d+5})});j.browser.msie&&parseInt(j.browser.version);j(".tg-timeline-envelope").each(function(){g=j(this);i=g.offset().left-B;h=g.find(".titleBar");a=h.data("lef");u=-1*(a+i);r=h.find(".timeline-title");u>0&&u<h.width()-r.width()&&r.css({marginLeft:u+5})})},registerDragging:function(){var d=q.startSec,l=j(t).position().left,
p=q.getZoomInfo().spp;d=new D(d-l*p);q.setFocusDate(d);this.displayFocusDate()},setupFilter:function(){var d=this;j(d._views.FILTER_BT);var l=j.tmpl(d._templates.filter_modal,{}).appendTo(d._views.CONTAINER);l.position({my:"right bottom",at:"right top",of:j(d._views.FILTER_BT),offset:"-8, -30"}).css("z-index",d.ztop++).hide();j(n).delegate(".timeglider-filter-box .close-button","click",function(){l.fadeOut()});j(d._views.FILTER_BT).bind("click",function(){l.fadeIn();j(this);var p=d._views.FILTER_BOX;
if(d.filterBoxActivated==false){d.filterBoxActivated=true;var b=j(p+" .timeglider-filter-apply"),a=j(".timeglider-filter-box .close-button"),k=j(p+" .timeglider-filter-clear"),g="",i="";b.bind("click",function(){g=j(p+" .timeglider-filter-include").val();i=j(p+" .timeglider-filter-exclude").val();q.setFilters({origin:"clude",include:g,exclude:i});j(p).toggleClass("timeglider-display-block")});a.bind("click",function(){j(p).toggleClass("timeglider-display-none")});k.bind("click",function(){q.setFilters({origin:"clude",
include:"",exclude:""});j(p+" .timeglider-filter-include").val("");j(p+" .timeglider-filter-exclude").val("");j(p).toggleClass("timeglider-display-block")})}})},buildTimelineMenu:function(){var d=this,l;j(d._views.TIMELINE_LIST_BT);j(d._views.TIMELINE_MENU)[0]&&j(d._views.TIMELINE_MENU).remove();l=j.tmpl(d._templates.timeline_list_modal,{}).appendTo(d._views.CONTAINER);var p=Backbone.View.extend({initialize:function(){this.model.bind("change",this.render,this)},tagName:"li",className:"timeglider-timeline-list-item",
template:"${title}",events:{click:"toggleTimeline"},toggleTimeline:function(){q.toggleTimeline(this.model.get("id"))},render:function(){var b=this.model.get("id");j(this.el).html(j.tmpl(this.template,this.model.attributes)).data("timeline_id",b);return this}});j(d._views.TIMELINE_MENU_UL).html("");_.each(q.timelineCollection.models,function(b){j(d._views.TIMELINE_MENU_UL).append((new p({model:b})).render().el)});l.position({my:"right bottom",at:"right top",of:j(d._views.TIMELINE_LIST_BT),offset:"-8, -30"}).hide();
j(n).delegate(".timeglider-timeline-menu .close-button","click",function(){l.fadeOut()}).delegate(this._views.TIMELINE_LIST_BT,"click",function(){l.fadeIn()})},getTimezonePulldown:function(d,l){var p="<select name='timezone' id='"+d+"'>",b=false,a="selected";j.map(D.timezones,function(k){if(l==k.offset&&b==false){a="selected";b=true}else a="";p+="<option value='"+k.offset+"' "+a+">"+k.name+"</option>"});p+="</select>";return p},buildSettingsMenu:function(){var d=j.tmpl(this._templates.settings_modal,
{}).appendTo(this._views.CONTAINER),l=this.getTimezonePulldown("timeglider-settings-timezone",q.timeOffset.string);d.find(".timeglider-settings-timezone").append('<p>Make changes below, then click on "save". More settings options to come!</p>').append('<span class="settings-label">timezone:</span> '+l).append("<p style='clear:both'>&nbsp;</p><div class='btn success' id='timeglider-settings-save'>save</div>");d.position({my:"right bottom",at:"right top",of:j(this._views.SETTINGS_BT),offset:"-8, -30"}).hide();
j(n).delegate(".timeglider-settings-modal .close-button","click",function(){d.fadeOut()}).delegate(this._views.SETTINGS_BT,"click",function(){d.fadeIn()}).delegate("#timeglider-settings-save","click",function(){var p=j(n+" #timeglider-settings-timezone").val();q.setTimeoffset(p)})},buildSlider:function(){var d=q.getZoomLevel();if(E.min_zoom==E.max_zoom)j(this._views.SLIDER_CONTAINER).css("display","none");else{E.display_zoom_level==true&&j("<div>").appendTo(this._views.SLIDER_CONTAINER).addClass("timeglider-zoomlevel-display").html("&nbsp;");
var l=this;d=l.invSliderVal(d);var p=q.max_zoom,b=q.min_zoom;j(this._views.SLIDER).css("height",(1+p-b)*3).slider({steps:100,handle:j(".knob"),animate:300,orientation:"vertical",min:l.invSliderVal(p),max:l.invSliderVal(b),value:d,start:function(){l.sliderActive=true},stop:function(){l.sliderActive=false},change:function(){},slide:function(a,k){q.setZoomLevel(l.invSliderVal(k.value))}})}},getEventDateLine:function(d){var l="<span class='timeglider-dateline-startdate'>"+d.startdateObj.format("",true,
q.timeOffset)+"</span>",p="";if(d.span==true)p=" &ndash; <span class='timeglider-dateline-enddate'>"+d.enddateObj.format("",true,q.timeOffset)+"</span>";return l+p},eventHover:function(d,l){var p=j(".timeglider-event-hover-info");l.date_display!="no"&&p.position({my:"left bottom",at:"left top",of:d,offset:"1, -10",collision:"flip flip"}).html(this.getEventDateLine(l));d.addClass("tg-event-hovered")},eventUnHover:function(d){j(".timeglider-event-hover-info").css("left","-1000px");d.removeClass("tg-event-hovered")},
clearTicks:function(){this.tickNum=this.leftside=0;j(t).css("left",0);j(n+" .tg-timeline-envelope").remove();j(n+" .timeglider-tick").remove()},castTicks:function(){this.clearTicks();q.getZoomLevel();var d=q.getFocusDate(),l=q.getZoomInfo().width;l=Math.ceil(this.dimensions.container.width/l)+4;var p="l";q.setTicksReady(false);this.addTick({type:"init",focus_date:d});for(d=1;d<=l;d+=1){this.addTick({type:p});p=p=="l"?"r":"l"}q.setTicksReady(true);this.displayFocusDate()},addTick:function(d){var l=
0,p=0,b=p=0,a=0,k=0,g=k="",i={};k={};b={};var h={};k={};i="";h=q.getZoomInfo().unit;var r=q.getZoomInfo().width;a=q.getFocusDate();b=parseInt(this.dimensions.tick.top);a=q.addToTicksArray({type:d.type,unit:h},a);if(h=="mo"){k=D.getMonthAdj(a,r);r=k.width;l=k.days}this.tickNum++;if(d.type=="init"){k=this.tickOffsetFromDate(q.getZoomInfo(),q.getFocusDate(),r);this.leftside=p=Math.ceil(this.dimensions.container.centerx+k);this.rightside=p+r}else if(d.type=="l")this.leftside=p=Math.floor(this.leftside-
r);else if(d.type=="r"){p=Math.floor(this.rightside);this.rightside+=r}q.getTickBySerial(a).width=r;q.getTickBySerial(a).left=p;k=this._views.PLACE+"_"+h+"_"+a+"-"+this.tickNum;k=j("<div class='timeglider-tick' id='"+k+"'><div class='timeglider-tick-label' id='label'></div></div>").appendTo(t);k.css({width:r,left:p,top:b});b=this.getTickMarksInfo({unit:h,width:r});b=l||b.tperu;p=r/b;if(p>5)for(d=l=0;d<b;d++){g="&nbsp;";if(p>16)if(h=="da")g=this.getHourLabelFromHour(d,p);else if(h=="mo")g=d+1;else if(h==
"ye")g=p>30?"&nbsp;"+D.monthNamesAbbr[d+1]:"&nbsp;"+D.monthNamesLet[d+1];else if(h=="de"){if(p>44)g=a+""+d}else if(h=="ce")if(p>28)g=a+""+d+"0";i+="<div class='timeglider-tick-sub-label' style='left:"+l+"px;width:"+p+"px'>"+g+"</div>";l+=p}i&&k.append("<div style='background-color:none;height:24px;width:"+(r+10)+"px;position:absolute;top:15px;left:0;overflow:hidden'>"+i+"</div>");h={unit:h,width:r,serial:a};i=this.getDateLabelForTick(h);k.children("#label").text(i);return h},setCenterline:function(){q.options.show_centerline===
true?j(this._views.CENTERLINE).css({height:this.dimensions.container.height,left:this.dimensions.container.centerx}):j(this._views.CENTERLINE).css({display:"none"})},getHourLabelFromHour:function(d,l){var p="",b=d,a="";a=0;if(l<16)return"";else{if(d>12)b=d-12;else if(d==0)b=12;if(l>30)p=d>11?" pm":" am";if(l>200){a=l/4-4;return"<div class='minutes' style='width:"+a+"px'>"+b+":00 "+p+"</div><div class='minutes' style='width:"+a+"px'>"+b+":15 "+p+"</div><div class='minutes' style='width:"+a+"px'>"+
b+":30 "+p+"</div><div class='minutes' style='width:"+a+"px'>"+b+":45 "+p+"</div>"}else return b+(l>60?":00":"")+p}},getTickMarksInfo:function(d){switch(d.unit){case "da":d=24;break;case "mo":d=30;break;case "ye":d=12;break;default:d=10}return{tperu:d}},getDateLabelForTick:function(d){var l=d.serial,p=d.width;switch(d.unit){case "bill":return l==0?"1":l>0?l+" billion":l+" b.y. bce";case "hundredmill":return l==0?"1":l>0?l+"00 million":l+"00 m.y. bce";case "tenmill":return l==0?"1":l>0?l+"0 million":
l+"0 m.y. bce";case "mill":return l==0?"1":l>0?l+" million":l+" m.y. bce";case "hundredthou":return l==0?"1":l>0?l+"00,000":l+"00,000 bce";case "tenthou":return l==0?"1":l>0?l+"0,000":l+"0,000 bce";case "thou":return l==0?"1("+l+")":l>0?l+"000":l+"000 bce";case "ce":return l==0?"1("+l+")":l>0?l+"00":l+"00 bce";case "de":return l>120?l*10+"s":l*10;case "ye":return l;case "mo":d=D.getDateFromMonthNum(l);return p<120?D.monthNamesAbbr[d.mo]+" "+d.ye:D.monthNames[d.mo]+", "+d.ye;case "da":d=new D(D.getDateFromRD(l));
return p<120?D.monthNamesAbbr[d.mo]+" "+d.da+", "+d.ye:D.monthNames[d.mo]+" "+d.da+", "+d.ye;default:return d.unit+":"+l+":"+p}},tickHangies:function(){var d=j(t).position().left,l=this.rightside+d-this.dimensions.container.width;if(this.leftside+d>-100){d=this.addTick({type:"l"});this.appendTimelines(d)}else if(l<100){d=this.addTick({type:"r"});this.appendTimelines(d)}},tickOffsetFromDate:function(d,l,p){switch(d.unit){case "da":l=l.ho/24+l.mi/1440;p*=l;break;case "mo":d=D.getMonthDays(l.mo,l.ye);
l=(l.da-1)/d+l.ho/(24*d)+l.mi/(1440*d);p*=l;break;case "ye":l=((l.mo-1)*30+l.da)/365;p*=l;break;case "de":l=l.ye%10/10+l.mo/120;p*=l;break;case "ce":l=l.ye%100/100+l.mo/1200;p*=l;break;case "thou":l=l.ye%1E3/1E3;p*=l;break;case "tenthou":l=l.ye%1E4/1E4;p*=l;break;case "hundredthou":l=l.ye%1E5/1E5;p*=l;break;default:p=0}return-1*p},resetTicksHandle:function(){j(this._views.HANDLE).offset({left:j(n).offset().left})},easeOutTicks:function(){Math.abs(z)>5&&j(t).animate({left:"+="+3*z},1E3,"easeOutQuad",
function(){debug.trace("stopping easing","note")})},getTimelineEventsByTick:function(d){var l=d.tick.unit,p=d.tick.serial;d=q.eventCollection.getTimelineHash(d.timeline.timeline_id);return d[l][p]&&d[l][p].length>0?d[l][p]:0},passesFilters:function(d,l){var p=true,b="",a=[],k;b="";a=[];if(l<d.low_threshold||l>d.high_threshold)return false;if(b=q.filters.include){a=b.split(",");p=false;for(k=0;k<a.length;k++){b=RegExp(j.trim(a[k]),"i");if(d.title.match(b))p=true}}if(b=q.filters.exclude){a=b.split(",");
for(k=0;k<a.length;k++){b=RegExp(j.trim(a[k]),"i");if(d.title.match(b))p=false}}b=d.icon;if(q.filters.legend.length>0)if(j.inArray(b,q.filters.legend)==-1)p=false;return p},freshTimelines:function(){var d=this,l,p,b,a=q.activeTimelines,k=q.ticksArray,g="",i,h;p="";var r=d.dimensions.container.centerx;l=d.dimensions.container.width;var u=q.getFocusDate().sec,B=q.getZoomInfo().spp;q.getZoomInfo();p=[];var c=[],f=u-Math.floor(B*(l/2));b=i=0;for(var m,x=0,C=d.dimensions.container.height,F=x=0;F<a.length;F++){c=
[];p=q.timelineCollection.get(a[F]);l=p.attributes;m=l.display;x=l.top?parseInt(l.top.replace("px","")):C-d.initTimelineVOffset;p=new s.TG_TimelineView({model:p});i=q.timeOffset.seconds/B;h=j(p.render().el).appendTo(t);h.draggable({axis:"y",handle:".titleBar",stop:function(){var I=j(this).css("top"),M=j(this).attr("id");q.timelineCollection.get(M).set({top:I});q.refresh()}}).css({top:x,left:i});i=h.find(".titleBar");if(typeof l.bounds!="undefined"){p=r+(l.bounds.first-u)/B;b=r+(l.bounds.last-u)/B}else{p=
r;b=r+300}b-=p;i.css({top:void 0,left:p,width:b}).data({lef:p,wid:b});if(m=="expanded")l.borg=g=new timeglider.TG_Org;for(i=0;i<k.length;i++){p=this.getTimelineEventsByTick({tick:k[i],timeline:l});j.merge(c,p)}_.each(l.spans,function(I){I.start<f&&I.end>f&&j.inArray(I.id,c)==-1&&c.unshift(I.id)});p=this.compileTickEventsAsHtml(l,c,0,"sweep");x=E.event_overflow=="scroll"?0:l.hasImagesAbove?x-d.imageLaneHeight:x;if(m=="expanded"){p=g.getHTML("sweep",x);l.borg=g.getBorg()}p!="undefined"&&h.append(p.html);
setTimeout(function(){d.registerEventImages(h)},3)}d.registerTitles();j.publish("viewer.rendered")},appendTimelines:function(d){var l=q.activeTimelines,p=[],b;p="";for(var a=0;a<l.length;a++){b=q.timelineCollection.get(l[a]).attributes;p=this.getTimelineEventsByTick({tick:d,timeline:b});p=this.compileTickEventsAsHtml(b,p,d.serial,"append");if(b.display=="expanded")p=b.borg.getHTML(d.serial,b.top);b=j(n+" .tg-timeline-envelope#"+b.id).append(p.html);this.registerEventImages(b)}j.publish("viewer.rendered")},
compileTickEventsAsHtml:function(d,l,p,b){var a=0,k=this.dimensions.container.centerx,g=d.display,i="",h=q.startSec,r=q.getZoomInfo().spp,u=q.getZoomInfo().level,B=a=0,c=d.borg,f={},m="sweep";if(b=="append")m=p;for(p=0;p<l.length;p++){f=q.eventCollection.get(l[p]).attributes;if(this.passesFilters(f,u)===true){a=k+(f.startdateObj.sec-h)/r;if(g=="expanded"){b=d.size_importance!==false?this.scaleToImportance(f.importance,u):1;f.width=f.titleWidth*b+20;f.fontsize=this.basicFontSize*b;f.left=a;f.spanwidth=
0;if(f.span==true){f.spanwidth=(f.enddateObj.sec-f.startdateObj.sec)/r;if(f.spanwidth>f.width)f.width=f.spanwidth}a=Math.ceil(f.fontsize);f.height=a+2;f.top=0-a;if(f.image&&f.image.display_class==="inline"){b=(f.image.scale||100)/100;a=b*f.image.height+2;B=b*f.image.width+2;f.shape={img_ht:a,img_wi:B,top:f.top-a,bottom:f.top-4,left:f.left,right:f.left+B}}else f.shape=null;c.addBlock(f,m)}else if(g=="collapsed")i+="<div id='"+f.id+"' class='timeglider-event-collapsed' style='top:-2px;left:"+a+"px'></div>"}}return g==
"collapsed"?{html:i}:""},registerEventImages:function(d){j(n+" .timeglider-event-image-bar").each(function(){j(this).position({my:"top",at:"bottom",of:d,offset:"0, 0"}).css("left",0)});j(n+" .timeglider-event-image-above").each(function(){j(this).css("display","block").position({my:"top",at:"top",of:j(n),offset:"0, 12"}).css({left:0})})},expandCollapseTimeline:function(d){d=q.timelineCollection.get(d).attributes;d.display=d.display=="expanded"?"collapsed":"expanded";q.refresh()},openTimelineModal:function(d){d=
new this.timelineModal({model:q.timelineCollection.get(d)});j(d.render().el).appendTo("body").position({my:"left top",at:"left top",of:this._views.CONTAINER,offset:"32, 32",collision:"fit fit"}).css("z-index",this.ztop++).draggable({stack:".timeglider-modal"})},createEventLinksMenu:function(d){if(!d)return"";var l="",p=0,b="",a="";if(typeof d=="string")l="<li><a href='"+d+"' target='_blank'>link</a></li>";else if(typeof d=="object")for(p=0;p<d.length;p++){b=d[p].url;a=d[p].label;l+="<li><a href='"+
b+"' target='_blank'>"+a+"</a></li>"}return l},eventModal:function(d){j(n+" #"+d+"_modal").remove();var l=this,p=false,b=false,a="";a={};a=0;a=j("#"+d);var k=q.eventCollection.get(d).attributes,g=k.modal_type||E.event_modal.type,i=k.image&&k.image.src?"<img src='"+k.image.src+"'>":"",h=this.createEventLinksMenu(k.link);d={title:k.title,description:k.description,id:d,dateline:l.getEventDateLine(k),links:h,image:i};if(k.video){d.video=k.video;g="full";b=true;d.video=k.video}else if(k.map&&k.map.latlong){p=
true;g="full"}switch(g){case "full":d=j.tmpl(l._templates.event_modal_full,d);d.appendTo(n).css({"z-index":l.ztop++}).position({my:"left top",at:"left top",of:n,offset:"0, 0",collision:"none none"});if(p==true){d.find("#insert").append("<div id='map_modal_map'></div>");a=k.map.zoom||12;p=String(k.map.latlong).split(",");p=new google.maps.LatLng(parseFloat(p[0]),parseFloat(p[1]));a={zoom:a,center:p,mapTypeId:google.maps.MapTypeId.ROADMAP};a=new google.maps.Map(j("#map_modal_map")[0],a);if(k.map.markers)for(p=
0;p<k.map.markers.length;p++){b=k.map.markers[p];new google.maps.MarkerImage(b.image,new google.maps.Size(24,32),new google.maps.Point(0,0),new google.maps.Point(0,32));d=b.latlong.split(",");d=new google.maps.LatLng(d[0],d[1]);b=new google.maps.Marker({position:d,map:a,icon:b.icon,title:b.title,zIndex:b.zIndex})}}else b==true&&d.find("#insert").append("<iframe width='100%' height='300' src='"+k.video+"'></iframe></div>");k=l.dimensions.container.height;p=l.dimensions.container.width;b=j(n+" .full_modal_panel");
d=b.width();a=b.height();b.css({top:(k-a)/2+"px",left:(p-d)/2+"px"});break;case "video":d=j.tmpl(l._templates.event_modal_video,d);d.appendTo(t).css("z-index",l.ztop++).position({my:"right center",at:"left center",of:a,offset:"-12, -1",collision:"flip fit"}).hover(function(){j(this).css("z-index",l.ztop++)});break;default:d=j.tmpl(l._templates.event_modal_small,d);d.appendTo(t).css("z-index",l.ztop++).position({my:"center top",at:"center top",of:n,offset:"0, 100",collision:"fit fit"}).draggable().hover(function(){j(this).css("z-index",
l.ztop++)})}},legendModal:function(d){var l=q.timelineCollection.get(d).attributes.legend,p=0,b="",a="",k="";p="";for(p=0;p<l.length;p++){b=E.icon_folder+l[p].icon;a=l[p].title;k+="<li><img class='legend-icon' src='"+b+"'><span class='legend-info'>"+a+"</span></li>"}d={id:d,legend_list:k};j(n+" .timeglider-legend").remove();j.tmpl(this._templates.legend_modal,d).appendTo(n).css("z-index",this.ztop++).toggleClass("timeglider-display-none").position({my:"left top",at:"left top",of:n,offset:"16, -4",
collision:"none none"});p=n+" .legend-info, "+n+" .legend-icon";j(p).bind("mouseup",function(g){g=j(g.target).parent().children("img").attr("src");j(this).parent().toggleClass("tg-legend-icon-selected");q.setFilters({origin:"legend",icon:g})})},parseHTMLTable:function(d){var l={},p=+new Date,b=[];j("#"+d).find("tr").each(function(a){var k=j(this).children(),g;if(a===0)b=k.map(function(){return j(this).attr("class").replace(/^.*?\btg-(\S+)\b.*?$/,"$1")}).get();else{g={};k.each(function(i){g[b[i]]=
j(this).text()});l["prefix"+p++]=g}});return l}};s.TG_TimelineView=Backbone.View.extend({initialize:function(){var d=this;this.model.bind("change:title",function(){j(d.el).find(".timeline-title-span").text(d.model.get("title"))});this.model.bind("destroy",this.remove,this)},tagName:"div",events:{"click .timeline-title-span":"titleClick"},className:"tg-timeline-envelope",getTemplate:function(){var d="<div class='titleBar'><div class='timeline-title'><span class='timeline-title-span'>${title}</span><div class='tg-timeline-env-buttons'>";
if(this.model.get("description"))d+="<span class='timeline-info' data-timeline_id='${id}'>info</span>";if(this.model.get("hasLegend"))d+="<span class='tg-timeline-legend-bt' data-timeline_id='${id}'>legend</span>";d+="<span class='expand-collapse' data-timeline_id='${id}'>exp/col</span>";d+="</div></div></div>";return d},render:function(){this.model.get("id");this.model.get("title");var d=this.getTemplate();j(this.el).html(j.tmpl(d,this.model.attributes)).attr("id",this.model.get("id"));return this},
setText:function(){},titleClick:function(){q.timelineTitleClick(this.model.get("id"))},remove:function(){j(this.el).remove()}});s.zoomTree=[{},{unit:"da",width:35E3,level:1,label:"5 hours"},{unit:"da",width:17600,level:2,label:"7 hours"},{unit:"da",width:8800,level:3,label:"10 hours"},{unit:"da",width:4400,level:4,label:"12 hours"},{unit:"da",width:2200,level:5,label:"14 hours"},{unit:"da",width:1100,level:6,label:"17 hours"},{unit:"da",width:550,level:7,label:"22 hours"},{unit:"da",width:432,level:8,
...
label:"15 million years"},{unit:"bill",width:358,level:97,label:"30 million years"},{unit:"bill",width:213,level:98,label:"50 million years"},{unit:"bill",width:126,level:99,label:"80 million years"},{unit:"bill",width:76,level:100,label:"100 billion years"}];s.calculateSecPerPx=function(d){for(var l=1;l<d.length;l++){var p=d[l],b=0;switch(p.unit){case "da":b=86400;break;case "mo":b=2419200;break;case "ye":b=31536E3;break;case "de":b=31536E4;break;case "ce":b=31536E5;break;case "thou":b=31536E6;break;
case "tenthou":b=31536E7;break;case "hundredthou":b=31536E8;break;case "mill":b=31536E9;break;case "tenmill":b=31536E10;break;case "hundredmill":b=31536E11;break;case "bill":b=31536E12}p.spp=b/parseInt(p.width)}}(s.zoomTree);s.getStringWidth=function(d){if(d)return j("#timeglider-measure-span").html(d).width()};String.prototype.removeWhitespace=function(){return this.replace(RegExp("\\n","g"),"")};if(debug)debug.trace=function(d,l){j("#"+l).text(d)};s.googleMapsInit=function(){};s.googleMapsLoaded=
