...
0||s.substr(0,1)=="b")e=-1*Math.abs(e);return C.validateDate(e,z,q)?e+"-"+zb(z)+"-"+zb(q):false}else return false};C.transValidateTimeString=function(e){if(!e)return"12:00:00";e=e.toLowerCase().match(/^(\d{1,2}|noon):?(\d{1,2})?:?(\d{1,2})? ?(am|pm)?/i);var q="";q=C.zeroButt;if(e[1])if(e[0]=="noon")q="12:00:00";else{var s=parseInt(e[1],10)||12,z=parseInt(e[2],10)||0,g=parseInt(e[3],10)||0;e=e[4]||"am";if(C.validateTime(s,z,g)==false)return false;if(e=="pm"&&s<12)s+=12;q=q(s)+":"+q(z)+":"+q(g)}else q=
false;return q};C.validateTime=function(e,q,s){if(e<0||e>23||q<0||q>59||s<0||s>59)return false;return true};C.validateDate=function(e,q,s){var z=C.getMonthDays(q,e);if(s>z||s<=0)return false;if(q>12||q<0)return false;if(e==0)return false;return true};C.zeroButt=function(e){e=parseInt(e,10);return e>9?String(e):"0"+e};C.toFromUTC=function(e,q,s){var z=0,g=[0,31,28,31,30,31,30,31,31,30,31,30,31,29];z={};if(s=="from"){z.ho=-1*q.hours;z.mi=-1*q.minutes}else if(s=="to"){z.ho=q.hours;z.mi=q.minutes}else{z.ho=
-1*e.tz_ho;z.mi=-1*e.tz_mi}if(z.ho==0&&z.mi==0)return e;s=e.ho+e.mi/60+(-1*z.ho+z.mi*-1/60);if(s<0){z=24+s;if(e.da>1)e.da-=1;else if(e.mo==1){e.ye-=1;e.mo=12;e.da=31}else{e.mo-=1;e.da=C.getLastDayOfMonth(e.ye,e.mo)}}else if(s>=24){z=s-24;if(C.isLeapYear(e.ye)&&e.mo==2&&e.da==28)e.da=29;else if(e.da==g[e.mo]){if(e.mo==12){e.ye+=1;e.mo=1}else e.mo+=1;e.da=1}else e.da+=1}else z=s;g=function(k){k=Math.abs(k);var p=Math.floor(k);return{ho:p,mi:Math.round((k-p)*60),se:0}}(z);e.ho=g.ho;e.mi=g.mi;if(q){e.tz_ho=
...
start:function(){},drag:function(){x=Math.floor(h(this).position().left);o.setTicksOffset(x);B=x-w;w=x},stop:function(){p.resetTicksHandle();p.registerDragging()}}).delegate(e+" .timeglider-timeline-event",z,function(){var c=h(this).attr("id"),a=o.eventCollection.get(c).attributes;if(timeglider.mode!="authoring")if(a.click_callback){c=a.click_callback.split(".");var n=c.length;if(n==1)window[c[0]](a);else if(n==2)window[c[0]][c[1]](a);else if(n==3)window[c[0]][c[1]][c[2]](a)}else p.eventModal(c)}).delegate(".timeglider-timeline-event",
"mouseover",function(){var c=o.eventCollection.get(h(this).attr("id")).attributes;p.eventHover(h(this),c)}).delegate(".timeglider-timeline-event","mouseout",function(){var c=o.eventCollection.get(h(this).attr("id")).attributes;p.eventUnHover(h(this),c)}).delegate(".timeglider-event-collapsed","hover",function(){o.eventCollection.get(h(this).attr("id"))});p.setCenterline();h.subscribe("mediator.ticksOffsetChange",function(){p.tickHangies();p.registerDragging();p.registerTitles()});h.subscribe("mediator.focusToEvent",
function(){});h.subscribe("mediator.zoomLevelChange",function(){p.tickNum=0;p.leftside=0;var c=o.getZoomLevel();h(p._views.SLIDER).slider("value",p.invSliderVal(c));p.displayZoomLevel(c);p.castTicks("zoomLevelChange")});h.subscribe("viewer.rendered",function(){});h.subscribe("mediator.ticksReadySignal",function(){o.ticksReady===true&&p.freshTimelines()});h.subscribe("mediator.refreshSignal",function(){p.tickNum=0;p.leftside=0;p.castTicks("refreshSignal")});h.subscribe("mediator.ticksArrayChange",
function(){});h.subscribe("mediator.focusDateChange",function(){this.displayFocusDate()});h.subscribe("mediator.timelineDataLoaded",function(){h(".timeglider-loading").fadeOut(500);p.buildSettingsMenu();p.buildTimelineMenu()});h.subscribe("mediator.activeTimelinesChange",function(){h(p._views.TIMELINE_MENU_UL+" li").each(function(){var c=h(this).data("timeline_id");h.inArray(c,o.activeTimelines)!=-1?h(this).addClass("activeTimeline"):h(this).removeClass("activeTimeline")})});h.subscribe("mediator.filterChange",
function(){});if(h.support.touch){h("#"+C).addTouch();var E=document.getElementById(C);E.addEventListener("gesturestart",function(c){c.preventDefault();h("#output").append("<br>gesture zoom:"+o.getZoomLevel())},false);E.addEventListener("gestureend",function(c){c.preventDefault();h("#output").append("<br>gesture end:"+o.getZoomLevel())},false);E.addEventListener("gesturechange",function(c){c.preventDefault();c.preventDefault();if(o.gesturing===false){o.gesturing=true;o.gestureStartZoom=o.getZoomLevel()}o.setZoomLevel(Math.ceil(o.gestureStartZoom/
c.scale))},false)}};r.TG_PlayerView.prototype={resize:function(){var g=h(A).height();h(e).height(g);this.dimensions=this.getWidgetDimensions();o.setDimensions(this.dimensions);this.setCenterline();o.refresh()},getWidgetDimensions:function(){var g=h(e),k=g.width(),p=Math.floor(k/2)+1,E=g.height(),c=Math.floor(E/2),a=this.tick_height,n=g.position().left;g=g.offset();var f=H.show_footer==true?h(this._views.FOOTER).height():0;return{container:{width:k,height:E,centerx:p,centery:c,left:n,offset:g},tick:{top:E-
f-a},footer:{height:f}}},scaleToImportance:function(g,k){return g/k},displayZoomLevel:function(){var g=o.getZoomLevel();g>0&&H.display_zoom_level==true&&h(this._views.ZOOM_DISPLAY).text(g)},displayFocusDate:function(){var g=o.getFocusDate();h(s).text(g.format("d MMM yyyy",false))},setPanButton:function(g,k){var p=this;h(g).live("mousedown",function(){p.intervalMachine("pan",{type:"set",fn:p.pan,args:[k],intvl:33})}).live("mouseup",function(){p.intervalMachine("pan",{type:"clear",fn:p.pan,callback:"resetTicksHandle"})}).live("mouseout",
function(){p.intervalMachine("pan",{type:"clear",fn:p.pan,callback:"resetTicksHandle"})})},intervalMachine:function(g,k){var p=this;if(k.type==="clear"){clearInterval(I[g]);if(k.callback)p[k.callback]()}else I[g]=setInterval(function(){k.fn.apply(p,k.args)},k.intvl)},invSliderVal:function(g){return Math.abs(g-101)},pan:function(g){g=g||20;$t=h(q);newPos=$t.position().left+g;$t.css({left:newPos});o.setTicksOffset(newPos)},registerTitles:function(){var g,k,p,E,c,a,n,f,l,j=h(e).offset().left;h(".timeglider-event-spanning").each(function(){g=
h(this).offset().left-j;k=h(this).outerWidth();a=h(".timeglider-event-title",this);p=a.outerWidth()+5;E=a.siblings(".timeglider-event-spanner").outerWidth();E>p&&g<0&&Math.abs(g)<k-p&&a.css({marginLeft:-1*g+5})});h.browser.msie&&parseInt(h.browser.version);h(".tg-timeline-envelope").each(function(){n=h(this);f=n.offset().left-j;$tb=n.find(".titleBar");c=$tb.data("lef");l=-1*(c+f);$ti=$tb.find(".timeline-title");l>0&&l<$tb.width()-$ti.width()&&$ti.css({marginLeft:l+5})})},registerDragging:function(){var g=
...
h(this);var k=g._views.FILTER_BOX;if(g.filterBoxActivated==false){g.filterBoxActivated=true;var p=h(k+" .timeglider-filter-apply"),E=h(".timeglider-filter-box .close-button"),c=h(k+" .timeglider-filter-clear"),a="",n="";p.bind("click",function(){a=h(k+" .timeglider-filter-include").val();n=h(k+" .timeglider-filter-exclude").val();o.setFilters({origin:"clude",include:a,exclude:n});h(k).toggleClass("timeglider-display-block")});E.bind("click",function(){h(k).toggleClass("timeglider-display-none")});
c.bind("click",function(){o.setFilters({origin:"clude",include:"",exclude:""});h(k+" .timeglider-filter-include").val("");h(k+" .timeglider-filter-exclude").val("");h(k).toggleClass("timeglider-display-block")})}})},buildTimelineMenu:function(){var g=this,k;h(g._views.TIMELINE_LIST_BT);h(g._views.TIMELINE_MENU)[0]&&h(g._views.TIMELINE_MENU).remove();k=h.tmpl(g._templates.timeline_list_modal,{}).appendTo(g._views.CONTAINER);var p=Backbone.View.extend({initialize:function(){this.model.bind("change",
this.render,this)},tagName:"li",className:"timeglider-timeline-list-item",template:"${title}",events:{click:"toggleTimeline"},toggleTimeline:function(){o.toggleTimeline(this.model.get("id"))},render:function(){var E=this.model.get("id");h(this.el).html(h.tmpl(this.template,this.model.attributes)).data("timeline_id",E);return this}});h(g._views.TIMELINE_MENU_UL).html("");_.each(o.timelineCollection.models,function(E){h(g._views.TIMELINE_MENU_UL).append((new p({model:E})).render().el)});k.position({my:"right bottom",
at:"right top",of:h(g._views.TIMELINE_LIST_BT),offset:"-8, -12"}).hide();h(e).delegate(".timeglider-timeline-menu .close-button","click",function(){k.fadeOut()}).delegate(this._views.TIMELINE_LIST_BT,"click",function(){k.fadeIn()})},getTimezonePulldown:function(g,k){var p="<select name='timezone' id='"+g+"'>",E=false,c="selected";h.map(G.timezones,function(a){if(k==a.offset&&E==false){c="selected";E=true}else c="";p+="<option value='"+a.offset+"' "+c+">"+a.name+"</option>"});p+="</select>";return p},
buildSettingsMenu:function(){var g=h.tmpl(this._templates.settings_modal,{}).appendTo(this._views.CONTAINER),k=this.getTimezonePulldown("timeglider-settings-timezone",o.timeOffset.string);g.find(".timeglider-settings-timezone").append('<p>Make changes below, then click on "save". More settings options to come!</p>').append('<span class="settings-label">timezone:</span> '+k).append("<p style='clear:both'>&nbsp;</p><div class='btn success' id='timeglider-settings-save'>save</div>");g.position({my:"right bottom",
at:"right top",of:h(this._views.SETTINGS_BT),offset:"-8, -12"}).hide();h(e).delegate(".timeglider-settings-modal .close-button","click",function(){g.fadeOut()}).delegate(this._views.SETTINGS_BT,"click",function(){g.fadeIn()}).delegate("#timeglider-settings-save","click",function(){var p=h(e+" #timeglider-settings-timezone").val();o.setTimeoffset(p)})},buildSlider:function(){var g=o.getZoomLevel();if(H.min_zoom==H.max_zoom)h(this._views.SLIDER_CONTAINER).css("display","none");else{H.display_zoom_level==
true&&h("<div>").appendTo(this._views.SLIDER_CONTAINER).addClass("timeglider-zoomlevel-display").html("&nbsp;");var k=this;g=k.invSliderVal(g);var p=o.max_zoom,E=o.min_zoom;h(this._views.SLIDER).css("height",(1+p-E)*3).slider({steps:100,handle:h(".knob"),animate:300,orientation:"vertical",min:k.invSliderVal(p),max:k.invSliderVal(E),value:g,start:function(){k.sliderActive=true},stop:function(){k.sliderActive=false},change:function(){},slide:function(c,a){o.setZoomLevel(k.invSliderVal(a.value))}})}},
getEventDateLine:function(g){var k="<span class='timeglider-dateline-startdate'>"+g.startdateObj.format("",true,o.timeOffset)+"</span>";endDateF="";if(g.span==true)endDateF=" &ndash; <span class='timeglider-dateline-enddate'>"+g.enddateObj.format("",true,o.timeOffset)+"</span>";return k+endDateF},eventHover:function(g,k){var p=h(".timeglider-event-hover-info");k.date_display!="no"&&p.position({my:"left bottom",at:"left top",of:g,offset:"1, -10",collision:"flip flip"}).html(this.getEventDateLine(k));
g.addClass("tg-event-hovered")},eventUnHover:function(g){h(".timeglider-event-hover-info").css("left","-1000px");g.removeClass("tg-event-hovered")},clearTicks:function(){this.tickNum=this.leftside=0;h(q).css("left",0);h(e+" .tg-timeline-envelope").remove();h(e+" .timeglider-tick").remove()},castTicks:function(){this.clearTicks();o.getZoomLevel();var g=o.getFocusDate(),k=o.getZoomInfo().width;k=Math.ceil(this.dimensions.container.width/k)+4;var p="l";o.setTicksReady(false);this.addTick({type:"init",
focus_date:g});for(g=1;g<=k;g+=1){this.addTick({type:p});p=p=="l"?"r":"l"}o.setTicksReady(true);this.displayFocusDate()},addTick:function(g){var k=0,p=0,E=p=0,c=0,a=0,n=a="",f={};a={};E={};var l={};a={};f="";l=o.getZoomInfo().unit;var j=o.getZoomInfo().width;c=o.getFocusDate();E=parseInt(this.dimensions.tick.top);c=o.addToTicksArray({type:g.type,unit:l},c);if(l=="mo"){a=G.getMonthAdj(c,j);j=a.width;k=a.days}this.tickNum++;if(g.type=="init"){a=this.tickOffsetFromDate(o.getZoomInfo(),o.getFocusDate(),
j);this.leftside=p=Math.ceil(this.dimensions.container.centerx+a);this.rightside=p+j}else if(g.type=="l")this.leftside=p=Math.floor(this.leftside-j);else if(g.type=="r"){p=Math.floor(this.rightside);this.rightside+=j}o.getTickBySerial(c).width=j;o.getTickBySerial(c).left=p;a=this._views.PLACE+"_"+l+"_"+c+"-"+this.tickNum;a=h("<div class='timeglider-tick' id='"+a+"'><div class='timeglider-tick-label' id='label'></div></div>").appendTo(q);a.css({width:j,left:p,top:E});E=this.getTickMarksInfo({unit:l,
width:j});E=k||E.tperu;p=j/E;if(p>5)for(g=k=0;g<E;g++){n="&nbsp;";if(p>16)if(l=="da"&&p>16)n=this.getHourLabelFromHour(g,p);else if(l=="mo")n=g+1;f+="<div class='timeglider-tick-sub-label' style='left:"+k+"px;width:"+p+"px'>"+n+"</div>";k+=p}f&&a.append("<div style='background-color:none;height:24px;width:"+(j+10)+"px;position:absolute;top:15px;left:0;overflow:hidden'>"+f+"</div>");l={unit:l,width:j,serial:c};f=this.getDateLabelForTick(l);a.children("#label").text(f);return l},setCenterline:function(){o.options.show_centerline===
true?h(this._views.CENTERLINE).css({height:this.dimensions.container.height,left:this.dimensions.container.centerx}):h(this._views.CENTERLINE).css({display:"none"})},getHourLabelFromHour:function(g,k){var p="";p=p="";p=g>12?g-12:g;if(p==0)p=12;return p+(k>60?":00":"")+(g>11?" pm":" am")},getTickMarksInfo:function(g){switch(g.unit){case "da":g=24;break;case "mo":g=30;break;case "ye":g=12;break;default:g=10}return{tperu:g}},getDateLabelForTick:function(g){var k=g.serial,p=g.width;switch(g.unit){case "bill":return k==
0?"1":k>0?k+" billion":k+" b.y. bce";case "hundredmill":return k==0?"1":k>0?k+"00 million":k+"00 m.y. bce";case "tenmill":return k==0?"1":k>0?k+"0 million":k+"0 m.y. bce";case "mill":return k==0?"1":k>0?k+" million":k+" m.y. bce";case "hundredthou":return k==0?"1":k>0?k+"00,000":k+"00,000 bce";case "tenthou":return k==0?"1":k>0?k+"0,000":k+"0,000 bce";case "thou":return k==0?"1("+k+")":k>0?k+"000":k+"000 bce";case "ce":return k==0?"1("+k+")":k>0?k+"00":k+"00 bce";case "de":return k>120?k*10+"s":k*
10;case "ye":return k;case "mo":g=G.getDateFromMonthNum(k);return p<120?G.monthNamesAbbr[g.mo]+" "+g.ye:G.monthNames[g.mo]+", "+g.ye;case "da":g=new G(G.getDateFromRD(k));return p<120?G.monthNamesAbbr[g.mo]+" "+g.da+", "+g.ye:G.monthNames[g.mo]+" "+g.da+", "+g.ye;default:return g.unit+":"+k+":"+p}},tickHangies:function(){var g=h(q).position().left,k=this.rightside+g-this.dimensions.container.width;if(this.leftside+g>-100){g=this.addTick({type:"l"});this.appendTimelines(g)}else if(k<100){g=this.addTick({type:"r"});
this.appendTimelines(g)}},tickOffsetFromDate:function(g,k,p){switch(g.unit){case "da":k=k.ho/24+k.mi/1440;p*=k;break;case "mo":g=G.getMonthDays(k.mo,k.ye);k=(k.da-1)/g+k.ho/(24*g)+k.mi/(1440*g);p*=k;break;case "ye":k=((k.mo-1)*30+k.da)/365;p*=k;break;case "de":k=k.ye%10/10+k.mo/120;p*=k;break;case "ce":k=k.ye%100/100+k.mo/1200;p*=k;break;case "thou":k=k.ye%1E3/1E3;p*=k;break;case "tenthou":k=k.ye%1E4/1E4;p*=k;break;case "hundredthou":k=k.ye%1E5/1E5;p*=k;break;default:p=0}return-1*p},resetTicksHandle:function(){h(this._views.HANDLE).offset({left:h(e).offset().left})},
easeOutTicks:function(){Math.abs(B)>5&&h(q).animate({left:"+="+3*B},1E3,"easeOutQuad",function(){debug.trace("stopping easing","note")})},getTimelineEventsByTick:function(g){var k=g.tick.unit,p=g.tick.serial;g=o.eventCollection.getTimelineHash(g.timeline.timeline_id);return g[k][p]&&g[k][p].length>0?g[k][p]:0},passesFilters:function(g,k){var p=true,E="",c=[],a;E="";c=[];if(k<g.low_threshold||k>g.high_threshold)return false;if(E=o.filters.include){c=E.split(",");p=false;for(a=0;a<c.length;a++){E=RegExp(h.trim(c[a]),
"i");if(g.title.match(E))p=true}}if(E=o.filters.exclude){c=E.split(",");for(a=0;a<c.length;a++){E=RegExp(h.trim(c[a]),"i");if(g.title.match(E))p=false}}E=g.icon;if(o.filters.legend.length>0)if(h.inArray(E,o.filters.legend)==-1)p=false;return p},freshTimelines:function(){var g=this,k,p,E,c=o.activeTimelines,a=o.ticksArray,n="",f;p="";var l=g.dimensions.container.centerx;k=g.dimensions.container.width;var j=o.getFocusDate().sec,u=o.getZoomInfo().spp;o.getZoomInfo();p=[];var v=[],F=j-Math.floor(u*(k/
2));E=f=0;for(var d,i=0,m=g.dimensions.container.height,y=i=0;y<c.length;y++){v=[];p=o.timelineCollection.get(c[y]);k=p.attributes;d=k.display;i=k.top?parseInt(k.top.replace("px","")):m-g.initTimelineVOffset;p=new r.TG_TimelineView({model:p});f=o.timeOffset.seconds/u;$tl=h(p.render().el).appendTo(q);$tl.draggable({axis:"y",handle:".titleBar",stop:function(){var D=h(this).css("top"),J=h(this).attr("id");o.timelineCollection.get(J).set({top:D});o.refresh()}}).css({top:i,left:f});f=$tl.find(".titleBar");
if(typeof k.bounds!="undefined"){p=l+(k.bounds.first-j)/u;E=l+(k.bounds.last-j)/u}else{p=l;E=l+300}E-=p;f.css({top:void 0,left:p,width:E}).data({lef:p,wid:E});if(d=="expanded")k.borg=n=new timeglider.TG_Org;for(f=0;f<a.length;f++){p=this.getTimelineEventsByTick({tick:a[f],timeline:k});h.merge(v,p)}_.each(k.spans,function(D){D.start<F&&D.end>F&&h.inArray(D.id,v)==-1&&v.unshift(D.id)});p=this.compileTickEventsAsHtml(k,v,0,"sweep");i=H.event_overflow=="scroll"?0:k.hasImagesAbove?i-g.imageLaneHeight:
i;if(d=="expanded"){p=n.getHTML("sweep",i);k.borg=n.getBorg()}p!="undefined"&&$tl.append(p.html);setTimeout(function(){g.registerEventImages($tl)},3)}g.registerTitles();h.publish("viewer.rendered")},appendTimelines:function(g){for(var k=o.activeTimelines,p,E="",c=0;c<k.length;c++){p=o.timelineCollection.get(k[c]).attributes;idArr=this.getTimelineEventsByTick({tick:g,timeline:p});E=this.compileTickEventsAsHtml(p,idArr,g.serial,"append");if(p.display=="expanded")E=p.borg.getHTML(g.serial,p.top);p=h(e+
" .tg-timeline-envelope#"+p.id).append(E.html);this.registerEventImages(p)}h.publish("viewer.rendered")},compileTickEventsAsHtml:function(g,k,p,E){var c=0,a=this.dimensions.container.centerx,n=g.display,f=$tl.height();stuff="";foSec=o.startSec;spp=o.getZoomInfo().spp;zl=o.getZoomInfo().level;buffer=20;img_ht=0;borg=g.borg;block_arg="sweep";if(E=="append")block_arg=p;for(p=0;p<k.length;p++){ev=o.eventCollection.get(k[p]).attributes;if(this.passesFilters(ev,zl)===true){c=a+(ev.startdateObj.sec-foSec)/
spp;if(n=="expanded"){impq=g.size_importance!==false?this.scaleToImportance(ev.importance,zl):1;ev.width=ev.titleWidth*impq+buffer;ev.fontsize=this.basicFontSize*impq;ev.left=c;ev.spanwidth=0;if(ev.span==true){ev.spanwidth=(ev.enddateObj.sec-ev.startdateObj.sec)/spp;if(ev.spanwidth>ev.width)ev.width=ev.spanwidth}img_ht=0;c=Math.ceil(ev.fontsize);if(ev.image&&ev.image.display_class==="inline"){img_ht=(ev.image.scale||100)/100*ev.image.height+2;ev.width=ev.image.width>ev.width?ev.image.width:ev.width;
ev.shape={ev_ht:c,img_ht:img_ht+32}}ev.height=c+img_ht;ev.top=f-ev.height;borg.addBlock(ev,block_arg)}else if(n=="collapsed")stuff+="<div id='"+ev.id+"' class='timeglider-event-collapsed' style='top:"+(f-2)+"px;left:"+c+"px'></div>"}}return n=="collapsed"?stuff:""},registerEventImages:function(g){h(e+" .timeglider-event-image-bar").each(function(){h(this).position({my:"top",at:"bottom",of:g,offset:"0, 0"}).css("left",0)});h(e+" .timeglider-event-image-above").each(function(){h(this).css("display",
...
createEventLinksMenu:function(g){if(!g)return"";var k="",p=0,E="",c="";if(typeof g=="string")k="<li><a href='"+g+"' target='_blank'>link</a></li>";else if(typeof g=="object")for(p=0;p<g.length;p++){E=g[p].url;c=g[p].label;k+="<li><a href='"+E+"' target='_blank'>"+c+"</a></li>"}return k},eventModal:function(g){h(e+" #"+g+"_modal").remove();var k=this,p=false,E=false,c="";c={};c=0;c=h("#"+g);var a=o.eventCollection.get(g).attributes,n=a.modal_type||H.event_modal.type,f=a.image&&a.image.src?"<img src='"+
a.image.src+"'>":"",l=this.createEventLinksMenu(a.link);g={title:a.title,description:a.description,id:g,dateline:k.getEventDateLine(a),links:l,image:f};if(a.video){g.video=a.video;n="full";E=true;g.video=a.video}else if(a.map&&a.map.latlong){p=true;n="full"}switch(n){case "full":g=h.tmpl(k._templates.event_modal_full,g);g.appendTo(e).css({"z-index":k.ztop++}).position({my:"left top",at:"left top",of:e,offset:"0, 0",collision:"none none"});if(p==true){g.find("#insert").append("<div id='map_modal_map'></div>");
...
map:c,icon:E.icon,title:E.title,zIndex:E.zIndex})}}else E==true&&g.find("#insert").append("<iframe width='100%' height='300' src='"+a.video+"'></iframe></div>");a=k.dimensions.container.height;p=k.dimensions.container.width;E=h(e+" .full_modal_panel");g=E.width();c=E.height();E.css({top:(a-c)/2+"px",left:(p-g)/2+"px"});break;case "video":g=h.tmpl(k._templates.event_modal_video,g);g.appendTo(q).css("z-index",k.ztop++).position({my:"right center",at:"left center",of:c,offset:"-12, -1",collision:"flip fit"}).hover(function(){h(this).css("z-index",
k.ztop++)});break;default:g=h.tmpl(k._templates.event_modal_small,g);g.appendTo(q).css("z-index",k.ztop++).position({my:"center top",at:"center top",of:e,offset:"0, 100",collision:"fit fit"}).draggable().hover(function(){h(this).css("z-index",k.ztop++)})}},legendModal:function(g){var k=o.timelineCollection.get(g).attributes.legend,p=0,E="",c="",a="";p="";for(p=0;p<k.length;p++){E=H.icon_folder+k[p].icon;c=k[p].title;a+="<li><img class='legend-icon' src='"+E+"'><span class='legend-info'>"+c+"</span></li>"}g=
{id:g,legend_list:a};h(e+" .timeglider-legend").remove();h.tmpl(this._templates.legend_modal,g).appendTo(e).css("z-index",this.ztop++).toggleClass("timeglider-display-none").position({my:"left top",at:"left top",of:e,offset:"16, -4",collision:"none none"});p=timeglider.mode==="authoring"?e+" .legend-info":e+" .legend-info, "+e+" .legend-icon";h(p).bind("mouseup",function(n){n=h(n.target).parent().children("img").attr("src");h(this).parent().toggleClass("tg-legend-icon-selected");o.setFilters({origin:"legend",
icon:n})})},parseHTMLTable:function(g){var k={},p=+new Date,E=[];h("#"+g).find("tr").each(function(c){var a=h(this).children(),n;if(c===0)E=a.map(function(){return h(this).attr("class").replace(/^.*?\btg-(\S+)\b.*?$/,"$1")}).get();else{n={};a.each(function(f){n[E[f]]=h(this).text()});k["prefix"+p++]=n}});return k}};r.TG_TimelineView=Backbone.View.extend({initialize:function(){var g=this;this.model.bind("change:title",function(){h(g.el).find(".timeline-title-span").text(g.model.get("title"))});this.model.bind("destroy",
this.remove,this)},tagName:"div",events:{"click .timeline-title-span":"titleClick"},className:"tg-timeline-envelope",getTemplate:function(){var g="<div class='titleBar'><div class='timeline-title'><span class='timeline-title-span'>${title}</span><div class='tg-timeline-env-buttons'>";if(this.model.get("description"))g+="<span class='timeline-info' data-timeline_id='${id}'>info</span>";if(this.model.get("hasLegend"))g+="<span class='tg-timeline-legend-bt' data-timeline_id='${id}'>legend</span>";g+=
"<span class='expand-collapse' data-timeline_id='${id}'>exp/col</span>";g+="</div></div></div>";return g},render:function(){this.model.get("id");this.model.get("title");var g=this.getTemplate();h(this.el).html(h.tmpl(g,this.model.attributes)).attr("id",this.model.get("id"));return this},setText:function(){},titleClick:function(){o.timelineTitleClick(this.model.get("id"))},remove:function(){h(this.el).remove()}});r.zoomTree=[{},{unit:"da",width:35E3,level:1,label:"5 hours"},{unit:"da",width:17600,
...
h=H.inArray(x,this.activeTimelines);if(h==-1){this.activeTimelines.push(x);this.setFocusDate(new A(w.focus_date));this.setZoomLevel(w.initial_zoom)}else{this.activeTimelines.splice(h,1);this.refresh()}H.publish("mediator.activeTimelinesChange")},reportImageSize:function(x){var w=G.eventCollection.get(x.id);if(w.has("image")){if(x.error){w.attributes.image={};debug.log("WHOOPS: MISSING IMAGE: "+x.src)}else{w.attributes.image.width=x.width;w.attributes.image.height=x.height}this.imagesSized++;this.imagesSized==
