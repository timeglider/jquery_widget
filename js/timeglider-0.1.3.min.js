...
G.tzOffsetStr(g.enddate,B.timeOffset.string)}g.startdateObj=new G(g.startdate,g.date_display);if(g.enddate&&g.enddate!==g.startdate){g.enddateObj=new G(g.enddate,g.date_display);g.span=true;B.spans["s_"+g.id]={id:g.id,start:g.startdateObj.sec,end:g.enddateObj.sec}}else{g.enddateObj=g.startdateObj;g.span=false}if(g.image)if(g.image_class=="above"||g.image.display_class=="above")B.hasImagesAbove=true;B.startSeconds.push(g.startdateObj.sec);B.endSeconds.push(g.enddateObj.sec);g.cache={span:g.span,startdateObj:_.clone(g.startdateObj),
...
else if(m==3)window[a[0]][a[1]][a[2]](c)}else p.eventModal(a)}).delegate(".timeglider-timeline-event","mouseover",function(){var a=o.eventCollection.get(g(this).attr("id")).attributes;p.eventHover(g(this),a)}).delegate(".timeglider-timeline-event","mouseout",function(){var a=o.eventCollection.get(g(this).attr("id")).attributes;p.eventUnHover(g(this),a)}).delegate(".timeglider-event-collapsed","hover",function(){var a=o.eventCollection.get(g(this).attr("id")).attributes.title;debug.trace("collapsed, title:"+
a,"note")});p.setCenterline();g.subscribe("mediator.ticksOffsetChange",function(){p.tickHangies();p.registerDragging();p.registerTitles()});g.subscribe("mediator.focusToEvent",function(){});g.subscribe("mediator.zoomLevelChange",function(){p.tickNum=0;p.leftside=0;var a=o.getZoomLevel();g(p._views.SLIDER).slider("value",p.invSliderVal(a));p.displayZoomLevel(a);p.castTicks("zoomLevelChange")});g.subscribe("viewer.rendered",function(){});g.subscribe("mediator.ticksReadySignal",function(){o.ticksReady===
true&&p.freshTimelines()});g.subscribe("mediator.refreshSignal",function(){p.tickNum=0;p.leftside=0;p.castTicks("refreshSignal")});g.subscribe("mediator.ticksArrayChange",function(){});g.subscribe("mediator.focusDateChange",function(){this.displayFocusDate()});g.subscribe("mediator.timelineDataLoaded",function(){g(".timeglider-loading").fadeOut(500);p.buildSettingsMenu();p.buildTimelineMenu()});g.subscribe("mediator.activeTimelinesChange",function(){g(p._views.TIMELINE_MENU_UL+" li").each(function(){var a=
g(this).data("timeline_id");g.inArray(a,o.activeTimelines)!=-1?g(this).addClass("activeTimeline"):g(this).removeClass("activeTimeline")})});g.subscribe("mediator.filterChange",function(){});if(g.support.touch){g("#"+C).addTouch();var E=document.getElementById(C);E.addEventListener("gesturestart",function(a){a.preventDefault();g("#output").append("<br>gesture zoom:"+o.getZoomLevel())},false);E.addEventListener("gestureend",function(a){a.preventDefault();g("#output").append("<br>gesture end:"+o.getZoomLevel())},
false);E.addEventListener("gesturechange",function(a){a.preventDefault();a.preventDefault();if(o.gesturing===false){o.gesturing=true;o.gestureStartZoom=o.getZoomLevel()}o.setZoomLevel(Math.ceil(o.gestureStartZoom/a.scale))},false)}};r.TG_PlayerView.prototype={resize:function(){var h=g(A).height();g(e).height(h);this.dimensions=this.getWidgetDimensions();o.setDimensions(this.dimensions);this.setCenterline();o.refresh()},getWidgetDimensions:function(){var h=g(e),k=h.width(),p=Math.floor(k/2)+1,E=h.height(),
a=Math.floor(E/2),c=this.tick_height,m=h.position().left;h=h.offset();var f=H.show_footer==true?g(this._views.FOOTER).height():0;return{container:{width:k,height:E,centerx:p,centery:a,left:m,offset:h},tick:{top:E-f-c},footer:{height:f}}},scaleToImportance:function(h,k){return h/k},displayZoomLevel:function(){var h=o.getZoomLevel();h>0&&H.display_zoom_level==true&&g(this._views.ZOOM_DISPLAY).text(h)},displayFocusDate:function(){var h=o.getFocusDate();g(s).text(h.format("d MMM yyyy",false))},setPanButton:function(h,
k){var p=this;g(h).live("mousedown",function(){p.intervalMachine("pan",{type:"set",fn:p.pan,args:[k],intvl:33})}).live("mouseup",function(){p.intervalMachine("pan",{type:"clear",fn:p.pan,callback:"resetTicksHandle"})}).live("mouseout",function(){p.intervalMachine("pan",{type:"clear",fn:p.pan,callback:"resetTicksHandle"})})},intervalMachine:function(h,k){var p=this;if(k.type==="clear"){clearInterval(I[h]);if(k.callback)p[k.callback]()}else I[h]=setInterval(function(){k.fn.apply(p,k.args)},k.intvl)},
invSliderVal:function(h){return Math.abs(h-101)},pan:function(h){h=h||20;$t=g(q);newPos=$t.position().left+h;$t.css({left:newPos});o.setTicksOffset(newPos)},registerTitles:function(){var h,k,p,E,a,c,m,f,l,j=g(e).offset().left;g(".timeglider-event-spanning").each(function(){h=g(this).offset().left-j;k=g(this).outerWidth();c=g(".timeglider-event-title",this);p=c.outerWidth()+5;E=c.siblings(".timeglider-event-spanner").outerWidth();E>p&&h<0&&Math.abs(h)<k-p&&c.css({marginLeft:-1*h+5})});g.browser.msie&&
parseInt(g.browser.version);g(".tg-timeline-envelope").each(function(){m=g(this);f=m.offset().left-j;$tb=m.find(".titleBar");a=$tb.data("lef");l=-1*(a+f);$ti=$tb.find(".timeline-title");l>0&&l<$tb.width()-$ti.width()&&$ti.css({marginLeft:l+5})})},registerDragging:function(){var h=o.startSec,k=g(q).position().left,p=o.getZoomInfo().spp;h=new G(h-k*p);o.setFocusDate(h);this.displayFocusDate()},setupFilter:function(){var h=this;$filter=g.tmpl(h._templates.filter_modal,{}).appendTo(h._views.CONTAINER);
$filter.css("z-index",h.ztop++).position({my:"right bottom",at:"right top",of:g(h._views.FILTER_BT),offset:"-8, -12"}).hide();g(e).delegate(".timeglider-filter-box .close-button","click",function(){$filter.fadeOut()});g(h._views.FILTER_BT).bind("click",function(){$filter.fadeIn();g(this);var k=h._views.FILTER_BOX;if(h.filterBoxActivated==false){h.filterBoxActivated=true;var p=g(k+" .timeglider-filter-apply"),E=g(".timeglider-filter-box .close-button"),a=g(k+" .timeglider-filter-clear"),c="",m="";
p.bind("click",function(){c=g(k+" .timeglider-filter-include").val();m=g(k+" .timeglider-filter-exclude").val();o.setFilters({origin:"clude",include:c,exclude:m});g(k).toggleClass("timeglider-display-block")});E.bind("click",function(){g(k).toggleClass("timeglider-display-none")});a.bind("click",function(){o.setFilters({origin:"clude",include:"",exclude:""});g(k+" .timeglider-filter-include").val("");g(k+" .timeglider-filter-exclude").val("");g(k).toggleClass("timeglider-display-block")})}})},buildTimelineMenu:function(){var h=
this,k;g(h._views.TIMELINE_LIST_BT);g(h._views.TIMELINE_MENU)[0]&&g(h._views.TIMELINE_MENU).remove();k=g.tmpl(h._templates.timeline_list_modal,{}).appendTo(h._views.CONTAINER);var p=Backbone.View.extend({initialize:function(){this.model.bind("change",this.render,this)},tagName:"li",className:"timeglider-timeline-list-item",template:"${title}",events:{click:"toggleTimeline"},toggleTimeline:function(){o.toggleTimeline(this.model.get("id"))},render:function(){var E=this.model.get("id");g(this.el).html(g.tmpl(this.template,
this.model.attributes)).data("timeline_id",E);return this}});g(h._views.TIMELINE_MENU_UL).html("");_.each(o.timelineCollection.models,function(E){g(h._views.TIMELINE_MENU_UL).append((new p({model:E})).render().el)});k.position({my:"right bottom",at:"right top",of:g(h._views.TIMELINE_LIST_BT),offset:"-8, -12"}).hide();g(e).delegate(".timeglider-timeline-menu .close-button","click",function(){k.fadeOut()}).delegate(this._views.TIMELINE_LIST_BT,"click",function(){k.fadeIn()})},getTimezonePulldown:function(h,
k){var p="<select name='timezone' id='"+h+"'>",E=false,a="selected";g.map(G.timezones,function(c){if(k==c.offset&&E==false){a="selected";E=true}else a="";p+="<option value='"+c.offset+"' "+a+">"+c.name+"</option>"});p+="</select>";return p},buildSettingsMenu:function(){var h=g.tmpl(this._templates.settings_modal,{}).appendTo(this._views.CONTAINER),k=this.getTimezonePulldown("timeglider-settings-timezone",o.timeOffset.string);h.find(".timeglider-settings-timezone").append('<p>Make changes below, then click on "save". More settings options to come!</p>').append('<span class="settings-label">timezone:</span> '+
